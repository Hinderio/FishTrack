...
const FISH_GAME_EVOLUTION_THRESHOLDS=[0,500,1500,3500];
const FISH_GAME_SPECIES={
  pike:{keys:['hecht','pike'],base:'Schilfjäger',theme:'#ffb84d',accent:'#8ff0a7',evolutions:['Schnapper','Schilfschreck','Tiefenjäger','See-König'],flavor:'Lauert ruhig im Schilf und explodiert beim perfekten Moment.'},
  perch:{keys:['barsch','egli','perch'],base:'Streifenflitzer',theme:'#8ff0a7',accent:'#4ad7d1',evolutions:['Streifenling','Schwarmjäger','Kantenräuber','Stachel-Champion'],flavor:'Schnell, frech und immer dort, wo Bewegung im Wasser ist.'},
  zander:{keys:['zander'],base:'Tiefenblick',theme:'#66e7ff',accent:'#b7a0ff',evolutions:['Dämmerling','Tiefenblick','Nachtjäger','Schattenkönig'],flavor:'Liest die Kanten im Halbdunkel wie eine geheime Karte.'},
  trout:{keys:['forelle','trout'],base:'Stromspringer',theme:'#ff8ab4',accent:'#66e7ff',evolutions:['Flitzer','Stromspringer','Silberpfeil','Flusslegende'],flavor:'Elegant im Strom, hellwach bei jeder Strömungskante.'},
  carp:{keys:['karpfen','karpfe','carp'],base:'Grundwächter',theme:'#b7a0ff',accent:'#ffb84d',evolutions:['Gründler','Grundwächter','Moosrücken','Teich-Titan'],flavor:'Geduldig am Grund, stark wenn es zählt.'},
  catfish:{keys:['wels','catfish'],base:'Nachtkoloss',theme:'#a6b0bd',accent:'#ff8ab4',evolutions:['Bartling','Nachtwächter','Schlammkoloss','Urgrund-Legende'],flavor:'Ein leiser Riese, der nachts seine Bahn zieht.'},
  default:{keys:[],base:'Uferling',theme:'#4ad7d1',accent:'#8ff0a7',evolutions:['Uferling','Revierfreund','Fanggeist','Spot-Legende'],flavor:'Wächst mit jedem echten Fang und jedem neuen Revier.'}
};
...
function computeCompanionEvolution(profile){
  ...
  let stageIndex=0;FISH_GAME_EVOLUTION_THRESHOLDS.forEach((min,i)=>{if(xp>=min)stageIndex=i;});
  const nextIndex=Math.min(stageIndex+1,FISH_GAME_EVOLUTION_THRESHOLDS.length-1);
  const currentMin=FISH_GAME_EVOLUTION_THRESHOLDS[stageIndex],nextMin=FISH_GAME_EVOLUTION_THRESHOLDS[nextIndex];
  const isMax=stageIndex===FISH_GAME_EVOLUTION_THRESHOLDS.length-1,span=Math.max(1,nextMin-currentMin);
  const theme=getFishGameSpeciesTheme(profile?.speciesKey);
  return{stage:stageIndex+1,totalStages:FISH_GAME_EVOLUTION_THRESHOLDS.length,name:theme.evolutions[stageIndex]||theme.base,nextName:isMax?null:(theme.evolutions[nextIndex]||theme.base),remaining:isMax?0:Math.max(0,nextMin-xp),percent:isMax?100:Math.max(0,Math.min(100,Math.round(((xp-currentMin)/span)*100))),isMax,level:Math.max(1,Math.floor(xp/170)+1)};
}
...
function computeParticipantPremiumBadges(profile,data,allStats=[],allCatches=[]){
  ...
  if(days.size>=3)add('streak-days','Fangserie',`${days.size} aktive Fangtage`,'rare','wave',42);
  if(profile.uniqueSpecies>=3)add('species-collector','Arten-Sammler',`${profile.uniqueSpecies} Fischarten entdeckt`,'rare','compass',48);
  ...
  if(trophyLen>=80||(globalLongest?.participantId===participantId&&trophyLen>0))add('trophy-touch','Trophy Touch',`${Math.round(trophyLen)} cm Highlight`,'epic','trophy',75);
  if(trophyLen>=100)add('meter-legend','Meter-Legende',`${Math.round(trophyLen)} cm Monsterfang`,'legendary','crown',120);
  const pike=speciesCount(['pike']);if(pike>=3)add('pike-hunter','Hechtjäger',`${pike} Hechte`,'rare','pike',45);
  const perch=speciesCount(['perch']);if(perch>=5)add('perch-king','Barschkönig',`${perch} Barsche/Egli`,'rare','perch',50);
  const zander=speciesCount(['zander']);if(zander>=3)add('zander-eye','Zanderblick',`${zander} Zander`,'rare','zander',45);
  const trout=speciesCount(['trout']);if(trout>=3)add('trout-dash','Forellenflitzer',`${trout} Forellen`,'rare','trout',45);
  const nightCount=catches.filter(c=>{const h=new Date(c.timestamp||c.createdAt||0).getHours();return h>=21||h<5}).length;
  if(nightCount>=2)add('night-angler','Nachtangler',`${nightCount} Nachtfänge`,'rare','moon',55);
  ...
  const duelWins=Number(profile.participant?.duelWins||profile.participant?.duel_wins||0);if(duelWins>0)add('duel-winner','Duellsieger',`${duelWins} gewonnene Duelle`,'epic','target',82);
  ...
  const sniperCount=catches.filter(fishGameIsSniperCatch).length;if(sniperCount>0)add('sniper-eye','Sniper-Auge',`${sniperCount} Zieltreffer`,'epic','target',86);
  if(profile.evolution?.stage>=4)add('legend-companion','Companion-Legende',profile.evolution.name,'legendary','crown',130);
...
function renderParticipantGameLayer(data,allStats=[],allCatches=[]){
  ...
  const visible=badges.slice(0,6),rest=badges.length-visible.length,nextLine=ev.isMax?'Maximale Evolution erreicht':`Noch ${ev.remaining} XP bis ${ev.nextName}`;
  return `<section class="participant-game-panel fish-game-panel"><article class="fish-companion-card"><div class="fish-companion-card__top"><div><p class="eyebrow">Companion Evolution</p><h3>${escapeHtml(ev.name)}</h3><span>Evolution ${ev.stage} / ${ev.totalStages}</span></div><div class="fish-companion-level"><strong>Lvl ${ev.level}</strong><small>${Number(profile.xp||0).toLocaleString('de-CH')} XP</small></div></div><div class="fish-companion-visual">${renderCompanionSvg(profile,154)}</div><div class="fish-companion-meta"><strong>${escapeHtml(profile.companionBase)}</strong><span>${escapeHtml(profile.flavor)}</span></div><div class="fish-xp-track" aria-label="${escapeHtml(nextLine)}"><div class="fish-xp-track__head"><span>${escapeHtml(nextLine)}</span><strong>${ev.percent}%</strong></div><div class="fish-xp-track__bar"><span style="width:${ev.percent}%"></span></div></div><div class="fishdex-mini"><span>FishDex: <b>${profile.uniqueSpecies}</b> Arten</span><span>Top-Art: <b>${escapeHtml(dominant)}</b></span><span>Rekord: <b>${escapeHtml(longest)}</b></span><span>Top-Gewicht: <b>${escapeHtml(heaviest)}</b></span></div></article><article class="fish-badge-collection"><div class="fish-badge-collection__head"><div><p class="eyebrow">Premium Badges</p><h3>Badge Collection</h3></div>${badges.length?`<span>${badges.length} freigeschaltet</span>`:''}</div><div class="fish-badge-grid">${visible.length?visible.map(renderPremiumBadgeCard).join(''):'<div class="fish-badge-empty">Deine Sammlung startet mit dem ersten Fang.<br>Trage einen Fang ein, um dein erstes Badge freizuschalten.</div>'}${rest>0?`<div class="fish-badge-more">+${rest} weitere freigeschaltet</div>`:''}</div><div class="fish-seniority-embed"><div class="participant-game-title"><span>Seniority</span>${renderSeniorityBadge(data.stats||data.participant)}</div>${renderSeniorityProgress(data.stats||data.participant)}</div></article></section>`;
}
...
(function(){
  ...
  function clearActiveSniperDuel(){localStorage.removeItem(SNIPER_ACTIVE_KEY);window.sniperDuelState=null;}
