...
state.catches = (catches || []).map(c => ({
  ...
  water_temp_c: c.water_temp_c ?? null,
  water_temp_source: c.water_temp_source ?? null,
  water_temp_fetched_at: c.water_temp_fetched_at ?? null
}));
...
async function saveCatchToSupabase(entry) {
  ...
  if (entry.weather_fetched_at) {
    payload.weather_temp_c = entry.weather_temp_c ?? null;
    payload.weather_feels_like_c = entry.weather_feels_like_c ?? null;
    payload.weather_wind_ms = entry.weather_wind_ms ?? null;
    payload.weather_humidity = entry.weather_humidity ?? null;
    payload.weather_clouds = entry.weather_clouds ?? null;
    payload.weather_precip_mm = entry.weather_precip_mm ?? null;
    payload.weather_condition = entry.weather_condition ?? null;
    payload.weather_icon = entry.weather_icon ?? null;
    payload.weather_fetched_at = entry.weather_fetched_at;
  }

  if (entry.water_temp_c != null) {
    payload.water_temp_c = Number(entry.water_temp_c);
    payload.water_temp_source = entry.water_temp_source || 'copernicus';
    payload.water_temp_fetched_at = entry.water_temp_fetched_at || new Date().toISOString();
  }

  // IMMER zuerst speichern - Wasser-/Weather-Enrichment darf den Catch-Insert nie blockieren
  ...
  if (entry.location?.lat != null && entry.location?.lng != null) {
    if (!entry.weather_fetched_at) {
      enrichCatchWeather(entry).catch(err => console.warn('Weather enrichment fehlgeschlagen', err));
    }
    if (entry.water_temp_c == null) {
      enrichCatchWaterTemp(entry).catch(err => console.warn('Water temp enrichment fehlgeschlagen', err));
    }
  }
}
...
if(data?.current){window.__lastWeatherData=data.current;}if(requestId!==weatherPopupRequestId)return;if(!weatherEnabled){setWeatherControlState();return}if(!data||!data.current){loadingPopup.setContent('<div class="weather-popup weather-popup--error">Open-Meteo konnte für diesen Punkt gerade keine Daten liefern.</div>');setWeatherControlState();return}loadingPopup.setContent(buildWeatherPopupHtml(data));setWeatherControlState()});map._weatherClickBound=true;}};async function enrichCatchWeather(entry){if(!db||!entry?.id||entry.weather_fetched_at)return null;const lat=entry.location?.lat,lng=entry.location?.lng;if(lat==null||lng==null)return null;try{const data=await getWeather(lat,lng);if(!data?.current)return null;const w=data.current;const weather={weather_temp_c:w.temperature_2m??null,weather_feels_like_c:w.apparent_temperature??null,weather_wind_ms:w.wind_speed_10m??null,weather_humidity:w.relative_humidity_2m??null,weather_clouds:w.cloud_cover??null,weather_precip_mm:w.precipitation??null,weather_condition:weatherDescription(w.weather_code),weather_icon:w.weather_code??null,weather_fetched_at:w.time??new Date().toISOString()};const {error}=await db.from('catches').update(weather).eq('id',entry.id);if(error){console.warn('Weather update fehlgeschlagen',error);return null}const target=(state.catches||[]).find(c=>c.id===entry.id);if(target){Object.assign(target,weather);localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}return weather}catch(e){console.warn('Weather enrichment fehlgeschlagen',e);return null}}
const WATER_TEMP_ENDPOINT=window.COPERNICUS_WATER_TEMP_ENDPOINT||'/copernicus/water-temp';async function fetchWaterTemp(lat,lng){if(lat==null||lng==null)return null;try{const params=new URLSearchParams({lat:String(lat),lng:String(lng)});const res=await fetch(`${WATER_TEMP_ENDPOINT}?${params.toString()}`);if(!res.ok)throw new Error(`Water temp ${res.status}`);const data=await res.json();const value=data?.water_temp_c??data?.temperature_c??data?.temperature??null;const n=Number(value);if(!Number.isFinite(n))return null;return{water_temp_c:n,water_temp_source:data?.source||'copernicus',water_temp_fetched_at:data?.fetched_at||new Date().toISOString()}}catch(e){console.warn('Water temp fetch fehlgeschlagen',e);return null}}async function enrichCatchWaterTemp(entry){if(!db||!entry?.id||entry.water_temp_c!=null)return null;const lat=entry.location?.lat,lng=entry.location?.lng;if(lat==null||lng==null)return null;const water=await fetchWaterTemp(lat,lng);if(!water||water.water_temp_c==null)return null;const {error}=await db.from('catches').update(water).eq('id',entry.id);if(error){console.warn('Water temp update fehlgeschlagen',error);return null}const target=(state.catches||[]).find(c=>c.id===entry.id);if(target){Object.assign(target,water);localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}return water}window.fetchWaterTemp=fetchWaterTemp;function loadState(){const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return structuredClone(defaultData);try{const data=JSON.parse(raw);return{meta:data.meta||structuredClone(defaultData.meta),participants:Array.isArray(data.participants)?data.participants:[],catches:Array.isArray(data.catches)?data.catches:[],tournaments:Array.isArray(data.tournaments)?data.tournaments:[]}}catch{return structuredClone(defaultData)}}
...
function usePredictionEnhancer(existingPrediction,bestPattern,relevantCount){
  ...
  return {...(existingPrediction||{}),best:bestPattern.catches[0],pattern:bestPattern,confidence:liveCatchConfidence(bestPattern,relevantCount),relevantCount};
}
...
async function updateLiveCatchPredictionLocation(lat,lng){
  ...
  if(typeof getWeather==='function'){
    ...
    liveCatchPredictionState.weather=data?.current||null;liveCatchPredictionState.loadingWeather=false;renderLiveCatchPrediction();
  }
}
