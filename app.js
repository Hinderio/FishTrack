...
e.target.reset();document.getElementById('customSpeciesWrap').classList.add('hidden');document.getElementById('timestampInput').value=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16);rerender();requestAnimationFrame(()=>showScreen('catches'))});document.getElementById('participantForm').addEventListener('submit',e=>{e.preventDefault();const form=e.target;const fd=new FormData(form),editingId=form.dataset.editingId;if(editingId){state.participants=state.participants.map(p=>p.id===editingId?{...p,name:fd.get('name').trim(),color:fd.get('color'),avatar:fd.get('avatar')||'🎣'}:p);delete form.dataset.editingId}else state.participants.push({id:crypto.randomUUID(),name:fd.get('name').trim(),color:fd.get('color'),avatar:fd.get('avatar')||'🎣'});persist();
...
function restoreCatchDropdowns(){
  const bait=document.querySelector('#catchForm [name="bait"]');
  if(bait && bait.tagName!=='SELECT'){
    bait.outerHTML=`<select name="bait">
      <option value="">Bitte wählen</option>
      ...
      <option value="Topwater-Köder">Topwater-Köder</option>
      <option value="Naturköder">Naturköder</option>
      <option value="Köderfisch">Köderfisch</option>
      <option value="Andere">Andere</option>
    </select>`;
  }

  const spot=document.querySelector('#catchForm [name="spotLabel"]');
  if(spot && spot.tagName!=='SELECT'){
    spot.outerHTML=`<select name="spotLabel">
      <option value="">Bitte wählen</option>
      <option value="Schilfkante">Schilfkante</option>
      <option value="Krautkante">Krautkante</option>
      <option value="Totholz">Totholz</option>
      <option value="Steilkante">Steilkante</option>
      <option value="Flachwasserbucht">Flachwasserbucht</option>
      <option value="Einlauf">Einlauf</option>
      <option value="Auslauf">Auslauf</option>
      <option value="Strömungskante">Strömungskante</option>
      <option value="Brückenpfeiler">Brückenpfeiler</option>
      <option value="Seerosenfeld">Seerosenfeld</option>
      <option value="Andere">Andere</option>
    </select>`;
  }
}
...
rerender = function(){
  originalRerender();
  restoreCatchDropdowns();
}
document.addEventListener('DOMContentLoaded',restoreCatchDropdowns);
