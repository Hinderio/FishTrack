...
function refreshAnalyticsTournamentSelect(){
  const select = document.getElementById('analyticsTournamentSelect');
  if(!select) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const originalRerender = window.rerender;
  if(typeof originalRerender === 'function' && !window.__analyticsRerenderWrapped){
    window.__analyticsRerenderWrapped = true;

    window.rerender = function(){
      originalRerender.apply(this, arguments);
      refreshAnalyticsTournamentSelect();
      rerenderAnalyticsCharts();
    };
  }
