...
/* Analytics Geo Maps – read-only Behaviour Layer extension */
const analyticsGeoState={heatMap:null,clusterMap:null,heatLayer:null,markersLayer:null,lastSignature:''};
function analyticsGeoCatches(){
  return [...(state?.catches||[])].filter(c=>c&&c.location&&Number.isFinite(Number(c.location.lat))&&Number.isFinite(Number(c.location.lng)));
}
function analyticsGeoSignature(items){
  return items.map(c=>[c.id,c.location.lat,c.location.lng,c.weightKg,c.lengthCm,c.timestamp].join(':')).join('|');
}
function analyticsGeoTileLayer(){
  return L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,crossOrigin:true});
}
function analyticsGeoEmpty(id,msg){
  const el=document.getElementById(id);
  if(el&&!el.querySelector('.analytics-geo-empty')) el.innerHTML=`<div class="analytics-geo-empty">${msg}</div>`;
}
function analyticsGeoBounds(items){
  const pts=items.map(c=>[Number(c.location.lat),Number(c.location.lng)]);
  return pts.length?L.latLngBounds(pts):null;
}
function analyticsFitGeoMap(map,items){
  if(!map||!items.length)return;
  const bounds=analyticsGeoBounds(items);
  if(bounds&&bounds.isValid()){
    if(items.length===1) map.setView(bounds.getCenter(),12,{animate:false});
    else map.fitBounds(bounds.pad(.22),{animate:false,maxZoom:12});
  }
}
function analyticsCreateHeatLayer(items){
  return L.Layer.extend({
    onAdd(map){this._map=map;this._canvas=L.DomUtil.create('canvas','analytics-heat-canvas');map.getPanes().overlayPane.appendChild(this._canvas);map.on('move zoom resize',this._draw,this);this._draw();},
    onRemove(map){map.off('move zoom resize',this._draw,this);if(this._canvas)this._canvas.remove();},
    _draw(){
      ...
      const ctx=canvas.getContext('2d');ctx.clearRect(0,0,size.x,size.y);
      if(!items.length)return;
      const weights=items.map(c=>Math.max(.55,Math.min(2.4,Number(c.weightKg||0)/3+.65)));
      const maxWeight=Math.max(1,...weights);
      items.forEach((c,i)=>{
        const p=map.latLngToContainerPoint([Number(c.location.lat),Number(c.location.lng)]);
        const w=weights[i]/maxWeight;
        const radius=Math.max(34,Math.min(76,42+map.getZoom()*2.2+w*18));
        const g=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,radius);
        const a=.14+w*.34;
        g.addColorStop(0,`rgba(218,255,235,${Math.min(.72,a+.18)})`);
        g.addColorStop(.25,`rgba(143,240,167,${a})`);
        g.addColorStop(.58,`rgba(74,215,209,${a*.58})`);
        g.addColorStop(1,'rgba(4,32,38,0)');
        ctx.fillStyle=g;ctx.beginPath();ctx.arc(p.x,p.y,radius,0,Math.PI*2);ctx.fill();
      });
    }
  })();
}
function analyticsBuildClusterGroups(items){
  const groups=new Map();
  items.forEach(c=>{
    const key=`${Number(c.location.lat).toFixed(2)},${Number(c.location.lng).toFixed(2)}`;
    if(!groups.has(key))groups.set(key,[]);
    groups.get(key).push(c);
  });
  return [...groups.values()];
}
function analyticsRenderGeoInsights(items){
  const box=document.getElementById('analyticsGeoInsightList');if(!box)return;
  if(!items.length){box.innerHTML='<div class="meta">Noch keine Koordinaten für Geo Insights vorhanden.</div>';return;}
  const spotCounts=analyticsCountBy(items,c=>c.spotLabel||c.location?.label||'Unbekannter Spot');
  const topSpot=spotCounts[0];
  const uniqueSpots=new Set(items.map(c=>c.spotLabel||c.location?.label||'Unbekannter Spot')).size;
  const totalWeight=items.reduce((s,c)=>s+Number(c.weightKg||0),0);
  const latest=[...items].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp))[0];
  box.innerHTML=`
    <div class="analytics-geo-insight"><strong>${items.length}</strong><span>Fänge mit gültigen Koordinaten fliessen in die Karten ein.</span></div>
    <div class="analytics-geo-insight"><strong>${escapeHtml(topSpot?.[0]||'–')}</strong><span>stärkster Geo-Spot mit ${topSpot?.[1]||0} Fang${topSpot?.[1]===1?'':'e'}.</span></div>
    <div class="analytics-geo-insight"><strong>${uniqueSpots}</strong><span>unterschiedliche Spot-Labels · ${fmtKg(totalWeight)} Gesamtgewicht im Geo-Sample.</span></div>
    <div class="analytics-geo-insight"><strong>${latest?escapeHtml(speciesName(latest)):'–'}</strong><span>letzter Fang mit Standort: ${latest?fmtDateTime(latest.timestamp):'keiner'}.</span></div>`;
}
function analyticsRenderGeoMaps(){
  if(typeof L==='undefined')return;
  const heatEl=document.getElementById('analyticsGeoHeatmap'),clusterEl=document.getElementById('analyticsGeoClusterMap');
  if(!heatEl||!clusterEl)return;
  const items=analyticsGeoCatches();
  document.getElementById('analyticsGeoHeatmapMeta')&&(document.getElementById('analyticsGeoHeatmapMeta').textContent=`${items.length} Standort${items.length===1?'':'e'}`);
  document.getElementById('analyticsGeoClusterMeta')&&(document.getElementById('analyticsGeoClusterMeta').textContent=`${analyticsBuildClusterGroups(items).length} Geo-Gruppe${analyticsBuildClusterGroups(items).length===1?'':'n'}`);
  analyticsRenderGeoInsights(items);
  if(!items.length){analyticsGeoEmpty('analyticsGeoHeatmap','Noch keine Fang-Koordinaten für die Heatmap vorhanden.');analyticsGeoEmpty('analyticsGeoClusterMap','Noch keine Fangpunkte mit Koordinaten vorhanden.');return;}
  const sig=analyticsGeoSignature(items);
  if(!analyticsGeoState.heatMap){heatEl.innerHTML='';analyticsGeoState.heatMap=L.map(heatEl,{zoomControl:true,scrollWheelZoom:false,dragging:true,preferCanvas:true});analyticsGeoTileLayer().addTo(analyticsGeoState.heatMap);}
  if(!analyticsGeoState.clusterMap){clusterEl.innerHTML='';analyticsGeoState.clusterMap=L.map(clusterEl,{zoomControl:true,scrollWheelZoom:false,dragging:true,preferCanvas:true});analyticsGeoTileLayer().addTo(analyticsGeoState.clusterMap);analyticsGeoState.markersLayer=L.layerGroup().addTo(analyticsGeoState.clusterMap);}
  if(sig!==analyticsGeoState.lastSignature){
    if(analyticsGeoState.heatLayer)analyticsGeoState.heatMap.removeLayer(analyticsGeoState.heatLayer);
    analyticsGeoState.heatLayer=analyticsCreateHeatLayer(items);analyticsGeoState.heatLayer.addTo(analyticsGeoState.heatMap);
    analyticsGeoState.markersLayer.clearLayers();
    analyticsBuildClusterGroups(items).forEach(group=>{
      const lat=group.reduce((s,c)=>s+Number(c.location.lat),0)/group.length,lng=group.reduce((s,c)=>s+Number(c.location.lng),0)/group.length;
      const icon=group.length>1?L.divIcon({className:'',html:`<div class="analytics-cluster-marker">${group.length}</div>`,iconSize:[38,38],iconAnchor:[19,19]}):L.divIcon({className:'',html:'<div class="analytics-catch-marker"></div>',iconSize:[18,18],iconAnchor:[9,9]});
      const top=[...group].sort((a,b)=>Number(b.weightKg||0)-Number(a.weightKg||0))[0];
      L.marker([lat,lng],{icon}).bindPopup(`<div class="analytics-geo-popup"><strong>${group.length} Fang${group.length===1?'':'e'} · ${escapeHtml(top.spotLabel||top.location?.label||'Spot')}</strong><span>Top: ${escapeHtml(speciesName(top))} · ${fmtKg(top.weightKg)} · ${Number(top.lengthCm||0).toFixed(0)} cm</span></div>`).addTo(analyticsGeoState.markersLayer);
    });
    analyticsFitGeoMap(analyticsGeoState.heatMap,items);analyticsFitGeoMap(analyticsGeoState.clusterMap,items);analyticsGeoState.lastSignature=sig;
  }
  setTimeout(()=>{analyticsGeoState.heatMap?.invalidateSize();analyticsGeoState.clusterMap?.invalidateSize();analyticsGeoState.heatLayer?._draw?.();},80);
}
const analyticsGeoPreviousRenderCharts=renderCharts;
renderCharts=function(){analyticsGeoPreviousRenderCharts.apply(this,arguments);analyticsRenderGeoMaps();};
const analyticsGeoPreviousShowScreen=showScreen;
showScreen=function(name){analyticsGeoPreviousShowScreen.apply(this,arguments);if(name==='analytics')setTimeout(analyticsRenderGeoMaps,140);};
window.addEventListener('resize',()=>setTimeout(analyticsRenderGeoMaps,120));
