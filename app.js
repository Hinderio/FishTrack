...
function rerenderAnalyticsPanels(){
  const originalCatches = state.catches;
...
// Final overrides: participant edit label, dual forecast cards, global analytics filter
setParticipantFormSubmitLabel(false);

getForecast=function(){const catches=state.catches||[];if(!catches.length)return{text7:'Noch zu wenig Daten für eine sinnvolle Prognose.',text30:'Noch zu wenig Daten für eine sinnvolle Prognose.'};const dates=catches.map(c=>startOfDay(c.timestamp).getTime()),min=Math.min(...dates),max=Math.max(...dates),days=Math.max(1,Math.round((max-min)/86400000)+1),avgPerDay=catches.length/days,projected7=Math.round(avgPerDay*7),projected30=Math.round(avgPerDay*30),avgWeight=catches.reduce((s,c)=>s+Number(c.weightKg||0),0)/catches.length;return{text7:`Wenn ihr dieses Tempo haltet, landet ihr in 7 Tagen bei etwa ${projected7} Fängen. Das entspricht rund ${fmtKg(projected7*avgWeight)} Gesamtgewicht.`,text30:`Wenn ihr dieses Tempo haltet, landet ihr in 30 Tagen bei etwa ${projected30} Fängen. Das entspricht rund ${fmtKg(projected30*avgWeight)} Gesamtgewicht.`,avgPerDay,projected7,projected30}};
renderForecast=function(){const forecast=getForecast(),box=document.getElementById('forecastBox');if(!box)return;box.innerHTML=`<article class="insight-card"><strong>7-Tage-Prognose</strong><span>${forecast.text7}</span></article><article class="insight-card"><strong>30-Tage-Prognose</strong><span>${forecast.text30}</span></article>`};

(function(){
  function analyticsScreenActive(){return document.getElementById('screen-analytics')?.classList.contains('active')}
  function withAnalyticsFilter(fn){
    if(typeof fn!=='function')return fn;
    return function(){
      if(!analyticsScreenActive())return fn.apply(this,arguments);
      const originalCatches=state.catches;
      state.catches=getAnalyticsCatches();
      try{return fn.apply(this,arguments)}finally{state.catches=originalCatches}
    };
  }
  ...
  const baseRefreshAnalyticsTournamentSelect=refreshAnalyticsTournamentSelect;
  refreshAnalyticsTournamentSelect=function(){
    if(typeof baseRefreshAnalyticsTournamentSelect==='function')baseRefreshAnalyticsTournamentSelect();
    const select=document.getElementById('analyticsTournamentSelect2');
    if(!select)return;
    if(!(state.tournaments||[]).some(t=>t.id===window.analyticsTournamentFilter))window.analyticsTournamentFilter='overview';
    select.value=window.analyticsTournamentFilter||'overview';
  };
  ...
  showScreen=function(name){
    ...
    if(name==='participants'){
      ...
      if(form&&!form.dataset.editingId)setParticipantFormSubmitLabel(false);
    }
    return result;
  };
})();
