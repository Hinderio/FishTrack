...
function computeParticipantMicroAwards(data,allStats=[],allCatches=[]){
  ...
  const todayCount=catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))).length;
  const maxToday=Math.max(0,...(allStats||[]).map(entry=>(entry.catches||[]).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))).length));
  ...
  const nightCount=catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).getHours();return h>=21||h<5}).length;
...
function participantTournamentSummary(participantId){
  ...
  tournaments.forEach(t=>{
    ...
    const participantCatches=(state.catches||[]).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).length;
  ...
  const played=entries.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).length;
  const wins=entries.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).length;
  const totalPoints=entries.reduce((sum,e)=>sum+e.points+e.bonus,0);
  const ranks=entries.map(e=>e.rank).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)&&n>0);
  const bestRank=ranks.length?Math.min(...ranks):null;
  const active=entries.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).sort((a,b)=>String(b.sortDate).localeCompare(String(a.sortDate)))[0]||null;
...
function renderParticipantComparePicker(participantId){
  const others=(state.participants||[]).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
...
function renderParticipantComparison(primaryId,secondaryId){
  ...
  const aWins=metrics.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).length,bWins=metrics.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).length;
...
const ratios=leaderboard
  .filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)
...
async function finishTournament(tournamentId){
  ...
  const winners=result.rows.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
  if(!winners.length){
  ...
  tournament.winner={
    ...
    participantIds:winners.map(w=>w.participant?.id).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0),
    points:topPoints,
...
(function() {
  ...
  const points = state.catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
        drawGridNew(points);
      } catch (e) {}
      return result;
    };
...
function getDashboardCatches(){
  ...
  return state.catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0) === window.dashboardTournamentFilter);
}
...
document.addEventListener('click', (e) => {
  ...
  const withZone = all.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
...
function getAnalyticsCatches(){
  ...
  return state.catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0) === window.analyticsTournamentFilter);
}
...
function analyticsAvg(values){
  const nums=values.map(Number).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
...
function buildPremiumAnalyticsModel(){
  ...
  const efficient=[...pStats].filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).map(p=>({...p,pointsPerCatch:p.points/p.count})).sort((a,b)=>b.pointsPerCatch-a.pointsPerCatch)[0];
...
function renderSpotBaitMatrix(){
  ...
  const max=Math.max(1,...spots.flatMap(spot=>baits.map(bait=>catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===spot&&(c.bait||'Unbekannt')===bait).length)));
  ...
  container.innerHTML=
    ...
    spots.map(spot=>'<div class="matrix-row"><div class="matrix-label">'+escapeHtml(spot)+'</div>'+baits.map(bait=>{
      const count=catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===spot&&(c.bait||'Unbekannt')===bait).length;
...
function renderParticipantTimeline(){
  ...
  const participants=[...new Set(state.catches.map(c=>participantById(c.participantId)?.name).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))];

  const datasets=participants.map((name,index)=>({
    label:name,
    data:state.catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)?.name===name).map(c=>({
...
function renderSpeciesTimeline(){
  ...
  const species=[...new Set(state.catches.map(c=>c.species||c.customSpecies||'Andere').filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))];

  const datasets=species.map((name,index)=>({
    label:name,
    data:state.catches
      .filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===name)
      .map(c=>({
        x:new Date(c.timestamp).getHours()+new Date(c.timestamp).getMinutes()/60,
        y:index+1,
...
function renderSpotBaitMatrix(){
  ...
  const spotTotals=Object.fromEntries(spots.map(s=>[s,catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===s).length]));
  const baitTotals=Object.fromEntries(baits.map(b=>[b,catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===b).length]));
  const values=spots.flatMap(spot=>baits.map(bait=>{
    const count=catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)===spot&&(c.bait||'Unbekannter Köder')===bait).length;
...
function renderParticipantTimeline(){
  ...
  const participants=[...new Set(catches.map(c=>participantById(c.participantId)?.name).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))];
  const datasets=participants.map((name,index)=>({
    label:name,
    data:catches.filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0)?.name===name).map(c=>({x:analyticsCatchHour(c)??0,y:index+1,r:Math.max(7,Math.min(22,7+Number(c.weightKg||0)*1.8)),species:speciesName(c),weight:Number(c.weightKg||0)})),
...
function renderAnalyticsPerformanceCharts(){
  ...
  if(efficiency){
    const data=[...pStats].filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0).sort((a,b)=>(b.points/b.count)-(a.points/a.count));
...
(function(){
  ...
  function geoCatches(){
    ...
    return (source||[]).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0))&&Number.isFinite(Number(c.location?.lng)));
  }
  ...
  function updateWeighted(mapInstance, points){
    ...
    const layer=L.layerGroup([...groups.values()].map(g=>{
      ...
      return L.circleMarker([g.lat,g.lng],{icon}).bindPopup(`<strong>${escapeHtml(g.label)}</strong><br>${g.count} Fang${g.count===1?'':'e'} · ${fmtKg(g.weight)}`);
    }));
  ...
  function renderGeoInsights(points){
    ...
    const weights=points.map(c=>Number(c.weightKg||0)).filter(c => c && typeof c.lat==="number" && typeof c.lng==="number" && c.lat!==0 && c.lng!==0);
...
const grouped = {};
(catches||[]).forEach(c=>{
  if(!c || typeof c.lat!=="number" || typeof c.lng!=="number" || c.lat===0 || c.lng===0) return;
  const key = c.lat + "," + c.lng;
  grouped[key] = (grouped[key]||0)+1;
});
const max = Math.max(1,...Object.values(grouped));
const heatPoints = Object.entries(grouped).map(([k,v])=>{
  const [lat,lng] = k.split(",").map(Number);
  return [lat,lng,v/max];
});
