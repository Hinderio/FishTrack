...
e.target.reset();document.getElementById('customSpeciesWrap').classList.add('hidden');document.getElementById('timestampInput').value=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16);rerender();showScreen('dashboard')});document.getElementById('participantForm').addEventListener('submit',e=>{e.preventDefault();const fd=new FormData(e.target);state.participants.push({id:crypto.randomUUID(),name:fd.get('name').trim(),color:fd.get('color'),avatar:fd.get('avatar')||'🎣'});persist();
...
/* === Combined fixes: participant edit + global analytics filter + 7 day forecast === */

(function(){

  // --- Participant edit ---
  const originalRenderParticipants = window.renderParticipants || renderParticipants;
  window.renderParticipants = function(){
    originalRenderParticipants();

    document.querySelectorAll('#participantsList .list-card').forEach(card => {
      if(card.querySelector('.edit-btn')) return;

      const title = card.querySelector('strong');
      const name = title ? title.textContent.replace(/^#?\d*\s*/, '').replace(/^[^\s]+\s/, '').trim() : '';

      const participant = (state.participants || []).find(p => name.includes(p.name));
  ...
  if(participantForm && !participantForm.dataset.editBound){
    participantForm.dataset.editBound = '1';

    participantForm.addEventListener('submit', (e) => {
      const editingId = participantForm.dataset.editingId;
      if(!editingId) return;

      e.preventDefault();

      const fd = new FormData(participantForm);
      state.participants = state.participants.map(p => p.id === editingId ? {
        ...p,
        name: fd.get('name'),
        color: fd.get('color'),
      ...
      delete participantForm.dataset.editingId;

      if(typeof persist === 'function') persist();
      participantForm.reset();
      participantForm.querySelector('[name="color"]').value = '#4ad7d1';
      participantForm.querySelector('[name="avatar"]').value = '🎣';
  ...
  window.getAnalyticsCatches = function(){
    if(window.analyticsTournamentFilter === 'overview'){
      return state.catches;
    }
    return state.catches.filter(c => (c.tournamentId || '') === window.analyticsTournamentFilter);
  };
  ...
  function withAnalyticsFilter(fn){
    return function(){
      const original = state.catches;
      if(document.getElementById('screen-analytics')?.classList.contains('active')){
        state.catches = window.getAnalyticsCatches();
      }
      try{
        return fn.apply(this, arguments);
      } finally {
  ...
  if(window.renderCharts) window.renderCharts = withAnalyticsFilter(window.renderCharts);
  if(window.renderTimeHeatmap) window.renderTimeHeatmap = withAnalyticsFilter(window.renderTimeHeatmap);
  if(window.renderRecords) window.renderRecords = withAnalyticsFilter(window.renderRecords);

  // --- 7 + 30 day forecast ---
  window.renderForecast = withAnalyticsFilter(function(){
    ...
    if(!catches.length){
      box.innerHTML = `
        <article class="insight-card"><strong>7-Tage-Prognose</strong><span>Noch zu wenig Daten für eine sinnvolle Prognose.</span></article>
        <article class="insight-card"><strong>30-Tage-Prognose</strong><span>Noch zu wenig Daten für eine sinnvolle Prognose.</span></article>
      `;
      return;
    }

    const dates = catches.map(c => new Date(c.timestamp).setHours(0,0,0,0));
    ...
    const catches7 = Math.round(avgPerDay * 7);
    const catches30 = Math.round(avgPerDay * 30);

    box.innerHTML = `
      <article class="insight-card">
        <strong>7-Tage-Prognose</strong>
        <span>Wenn ihr dieses Tempo haltet, landet ihr in 7 Tagen bei etwa ${catches7} Fängen. Das entspricht rund ${fmtKg(catches7 * avgWeight)} Gesamtgewicht.</span>
      </article>
      <article class="insight-card">
        <strong>30-Tage-Prognose</strong>
        <span>Wenn ihr dieses Tempo haltet, landet ihr in 30 Tagen bei etwa ${catches30} Fängen. Das entspricht rund ${fmtKg(catches30 * avgWeight)} Gesamtgewicht.</span>
      </article>
    `;
  });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
      if(window.refreshAnalyticsTournamentSelect) window.refreshAnalyticsTournamentSelect();
      if(window.renderParticipants) window.renderParticipants();
      if(window.renderForecast) window.renderForecast();
    }, 100);
  });
