...
window.state.tournaments = tournaments;
...
window.state.tournaments = tournaments;delete e.target.dataset.editingId}else state.tournaments.push(tournament);activeTournamentId=tournament.id;persist();
...
function reopenTournament(tournamentId){
  ...
  if(!confirm("Turnier wirklich wieder öffnen? Gewinner und Punkte werden gelöscht.")) return;
...
/* ===== Data Science Lab: isolated advanced analytics page ===== */
(function(){
  const dsCharts = {};
  window.dsTournamentFilter = window.dsTournamentFilter || 'overview';

  function textColor(){ return (typeof css === 'function' ? css('--text') : '#edf7ff') || '#edf7ff'; }
  function mutedColor(){ return (typeof css === 'function' ? css('--muted') : '#9fb2c7') || '#9fb2c7'; }
  function gridColor(){ return 'rgba(255,255,255,.08)'; }
  function palette(i){
    const p=['#4ad7d1','#8ff0a7','#ffb84d','#ff8ab4','#b7a0ff','#66e7ff','#f4d35e','#9ad1d4'];
    return p[i%p.length];
  }
  function dsCatches(){
    const all = state && Array.isArray(state.catches) ? state.catches : [];
    if(!window.dsTournamentFilter || window.dsTournamentFilter === 'overview') return all;
    return all.filter(c => (c.tournamentId || '') === window.dsTournamentFilter);
  }
  ...
  function destroy(id){ if(dsCharts[id]){ dsCharts[id].destroy(); delete dsCharts[id]; } }
  function makeChart(id, type, data, options={}){
    const el=document.getElementById(id);
    if(!el || typeof Chart === 'undefined') return;
    destroy(id);
    dsCharts[id]=new Chart(el, {type, data, options:{
      responsive:true, maintainAspectRatio:false,
      plugins:{legend:{labels:{color:textColor(), usePointStyle:true, boxWidth:8}}, tooltip:{enabled:true}, ...(options.plugins||{})},
      scales: options.scales,
      ...options
    }});
  }
  function refreshDsTournamentSelect(){
    const select=document.getElementById('dsTournamentSelect');
    if(!select) return;
    const current=window.dsTournamentFilter || 'overview';
    select.innerHTML='<option value="overview">Overview</option>'+(state.tournaments||[]).map(t=>`<option value="${t.id}">${t.name}</option>`).join('');
    if(current !== 'overview' && !(state.tournaments||[]).some(t=>t.id===current)) window.dsTournamentFilter='overview';
    select.value=window.dsTournamentFilter;
    if(select.dataset.bound==='1') return;
    select.dataset.bound='1';
    select.addEventListener('change',()=>{ window.dsTournamentFilter=select.value; renderDataScienceLab(); });
  }
  function renderKpis(catches){
    const el=document.getElementById('dsKpiGrid'); if(!el) return;
    const weights=catches.map(c=>Number(c.weightKg||0)).filter(Boolean);
    const lengths=catches.map(c=>Number(c.lengthCm||0)).filter(Boolean);
    const species=by(catches, c=>speciesName(c))[0]?.[0] || '–';
    const trophy=catches.reduce((m,c)=>!m||Number(c.lengthCm||0)>Number(m.lengthCm||0)?c:m,null);
    const activeDays=new Set(catches.map(c=>new Date(c.timestamp).toISOString().slice(0,10))).size;
    el.innerHTML=[
      ['Fänge', catches.length, 'Sample Size'],
      ['Ø Gewicht', fmtKg(avg(weights)), 'Mean'],
      ['Ø Länge', Math.round(avg(lengths)||0)+' cm', 'Distribution'],
      ['Top Art', species, 'Mode'],
      ['Trophy', trophy?`${speciesName(trophy)} ${trophy.lengthCm} cm`:'–', 'Outlier'],
      ['Aktive Tage', activeDays, 'Temporal Spread']
    ].map(([a,b,c],i)=>`<article class="ds-kpi glass"><span>${a}</span><strong>${b}</strong><small>${c}</small><i style="--i:${i}"></i></article>`).join('');
  }
  function renderTimeline(catches){
    const keys=[...new Set(catches.map(c=>new Date(c.timestamp).toISOString().slice(0,10)))].sort();
    const counts=keys.map(k=>catches.filter(c=>new Date(c.timestamp).toISOString().slice(0,10)===k).length);
    const avgW=keys.map(k=>avg(catches.filter(c=>new Date(c.timestamp).toISOString().slice(0,10)===k).map(c=>Number(c.weightKg||0))));
    makeChart('dsTimelineChart','line',{labels:keys.map(k=>k.slice(5)),datasets:[
      {label:'Fänge',data:counts,borderColor:palette(0),backgroundColor:'rgba(74,215,209,.16)',fill:true,tension:.42,pointRadius:4},
      {label:'Ø Gewicht kg',data:avgW,borderColor:palette(2),backgroundColor:'rgba(255,184,77,.12)',fill:true,tension:.42,pointRadius:3,yAxisID:'y1'}
    ]},{scales:{x:{ticks:{color:mutedColor()},grid:{display:false}},y:{ticks:{color:mutedColor()},grid:{color:gridColor()}},y1:{position:'right',ticks:{color:mutedColor()},grid:{display:false}}}});
  }
  function renderRadar(catches){
    const stats=computeParticipantStats().slice(0,5);
    makeChart('dsRadarChart','radar',{labels:['Fänge','Punkte','Ø Länge','Ø Gewicht','Trophy'],datasets:stats.map((p,i)=>({label:p.name,data:[p.count,p.points,Math.round(p.avgLength||0),Number((p.avgWeight||0).toFixed(1)),p.heaviest?Number(p.heaviest.lengthCm||0):0],borderColor:palette(i),backgroundColor:palette(i)+'33'}))},{scales:{r:{ticks:{display:false},grid:{color:gridColor()},angleLines:{color:gridColor()},pointLabels:{color:mutedColor()}}}});
  }
  function renderScatter(catches){
    const groups=by(catches,c=>speciesName(c)).map(x=>x[0]);
    makeChart('dsScatterChart','bubble',{datasets:groups.map((g,i)=>({label:g,data:catches.filter(c=>speciesName(c)===g).map(c=>({x:Number(c.lengthCm||0),y:Number(c.weightKg||0),r:Math.max(4,Math.min(18,Number(c.weightKg||0)*2+4))})),backgroundColor:palette(i)+'99',borderColor:palette(i)}))},{scales:{x:{title:{display:true,text:'cm',color:mutedColor()},ticks:{color:mutedColor()},grid:{color:gridColor()}},y:{title:{display:true,text:'kg',color:mutedColor()},ticks:{color:mutedColor()},grid:{color:gridColor()}}}});
  }
  function renderSimpleCharts(catches){
    const sp=by(catches,c=>speciesName(c));
    makeChart('dsDoughnutChart','doughnut',{labels:sp.map(x=>x[0]),datasets:[{data:sp.map(x=>x[1]),backgroundColor:sp.map((_,i)=>palette(i)),borderWidth:0}]},{cutout:'62%'});
    const hours=Array.from({length:24},(_,h)=>catches.filter(c=>new Date(c.timestamp).getHours()===h).length);
    makeChart('dsPolarChart','polarArea',{labels:hours.map((_,i)=>String(i).padStart(2,'0')),datasets:[{data:hours,backgroundColor:hours.map((_,i)=>palette(i)+'88'),borderWidth:0}]},{plugins:{legend:{display:false}}});
    const bait=by(catches.filter(c=>c.bait),c=>c.bait).slice(0,8);
    makeChart('dsBaitChart','bar',{labels:bait.map(x=>x[0]),datasets:[{label:'Score',data:bait.map(([b,n])=>n*avg(catches.filter(c=>c.bait===b).map(c=>Number(c.weightKg||0)+Number(c.lengthCm||0)/100))),backgroundColor:palette(1),borderRadius:12}]},{indexAxis:'y',plugins:{legend:{display:false}},scales:{x:{ticks:{color:mutedColor()},grid:{color:gridColor()}},y:{ticks:{color:mutedColor()},grid:{display:false}}}});
    const bins=[0,30,50,70,90,110,140], vals=bins.slice(0,-1).map((b,i)=>catches.filter(c=>Number(c.lengthCm||0)>=b && Number(c.lengthCm||0)<bins[i+1]).length);
    makeChart('dsHistogramChart','bar',{labels:bins.slice(0,-1).map((b,i)=>`${b}-${bins[i+1]} cm`),datasets:[{data:vals,backgroundColor:palette(3),borderRadius:12}]},{plugins:{legend:{display:false}},scales:{x:{ticks:{color:mutedColor()},grid:{display:false}},y:{ticks:{color:mutedColor()},grid:{color:gridColor()}}}});
    const names=state.participants.map(p=>p.name), data=state.participants.map(p=>new Set(catches.filter(c=>c.participantId===p.id).map(c=>new Date(c.timestamp).toISOString().slice(0,10))).size);
    makeChart('dsConsistencyChart','bar',{labels:names,datasets:[{label:'Aktive Fangtage',data,backgroundColor:names.map((_,i)=>palette(i)),borderRadius:12}]},{plugins:{legend:{display:false}},scales:{x:{ticks:{color:mutedColor()},grid:{display:false}},y:{ticks:{color:mutedColor()},grid:{color:gridColor()}}}});
  }
  function renderDomVisuals(catches){
    const tree=document.getElementById('dsTreemap');
    if(tree){ const sp=by(catches,c=>speciesName(c)); const total=Math.max(1,catches.length); tree.innerHTML=sp.map(([name,n],i)=>`<div class="ds-tree-cell" style="--w:${Math.max(18,n/total*100)}%;--c:${palette(i)}"><strong>${name}</strong><span>${n} Fänge</span></div>`).join('') || '<div class="meta">Keine Daten.</div>'; }
    const heat=document.getElementById('dsHeatmap');
    if(heat){ const days=['Mo','Di','Mi','Do','Fr','Sa','So']; let max=1; const vals=[]; for(let d=0;d<7;d++){ for(let h=0;h<24;h++){ const v=catches.filter(c=>{const dt=new Date(c.timestamp); return ((dt.getDay()+6)%7)===d && dt.getHours()===h}).length; vals.push([d,h,v]); max=Math.max(max,v); }} heat.innerHTML='<div></div>'+Array.from({length:24},(_,h)=>`<b>${h}</b>`).join('')+days.map((day,d)=>`<b>${day}</b>`+vals.filter(v=>v[0]===d).map(v=>`<span title="${day} ${v[1]}:00 · ${v[2]} Fänge" style="--o:${v[2]/max}"></span>`).join('')).join(''); }
    const out=document.getElementById('dsOutliers');
    if(out){ const sorted=[...catches].sort((a,b)=>(Number(b.lengthCm||0)*Number(b.weightKg||0))-(Number(a.lengthCm||0)*Number(a.weightKg||0))).slice(0,5); out.innerHTML=sorted.map((c,i)=>`<article class="list-card"><div><div class="list-title-row"><strong>#${i+1} ${speciesName(c)}</strong><span class="badge">${c.lengthCm} cm</span></div><div class="meta">${fmtKg(c.weightKg)} · ${participantById(c.participantId)?.name||'–'} · ${c.bait||'kein Köder'}</div></div></article>`).join('') || '<div class="meta">Keine Ausreisser vorhanden.</div>'; }
    const net=document.getElementById('dsNetwork');
    if(net){ const combos=by(catches.filter(c=>c.bait||c.spotLabel),c=>(c.spotLabel||'Spot?')+' × '+(c.bait||'Köder?')).slice(0,8); net.innerHTML=combos.map(([label,n],i)=>`<div class="ds-link-row"><span>${label}</span><strong>${n}</strong><em style="--w:${Math.min(100,n/Math.max(1,combos[0]?.[1]||1)*100)}%;--c:${palette(i)}"></em></div>`).join('') || '<div class="meta">Noch keine Spot/Köder-Kombinationen.</div>'; }
  }
  function renderDataScienceLab(){
    const screen=document.getElementById('screen-datascience');
    if(!screen) return;
    refreshDsTournamentSelect();
    const catches=dsCatches();
    renderKpis(catches);
    if(!catches.length){ renderDomVisuals(catches); ['dsTimelineChart','dsRadarChart','dsScatterChart','dsDoughnutChart','dsPolarChart','dsBaitChart','dsHistogramChart','dsConsistencyChart'].forEach(destroy); return; }
    renderTimeline(catches); renderRadar(catches); renderScatter(catches); renderSimpleCharts(catches); renderDomVisuals(catches);
  }
  window.renderDataScienceLab = renderDataScienceLab;
  const oldShow = showScreen;
  showScreen = function(name){
    const r = oldShow.apply(this, arguments);
    if(name === 'datascience') setTimeout(renderDataScienceLab, 0);
    return r;
  };
  const oldRerender = rerender;
  rerender = function(){
    const r = oldRerender.apply(this, arguments);
    if(document.getElementById('screen-datascience')?.classList.contains('active')) setTimeout(renderDataScienceLab,0);
    return r;
  };
