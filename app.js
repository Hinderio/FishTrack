...
e.target.reset();document.getElementById('customSpeciesWrap').classList.add('hidden');document.getElementById('timestampInput').value=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16);rerender();showScreen('dashboard')});document.getElementById('participantForm').addEventListener('submit',e=>{e.preventDefault();const fd=new FormData(e.target);const editingId=e.target.dataset.editingId;const participant={id:editingId||crypto.randomUUID(),name:fd.get('name').trim(),color:fd.get('color'),avatar:fd.get('avatar')||'🎣'};if(editingId){state.participants=state.participants.map(p=>p.id===editingId?participant:p);delete e.target.dataset.editingId}else{state.participants.push(participant)}persist();
