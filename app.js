...
state.catches = (catches || []).map(c => ({
  id: c.id,
  species: normalizeSpeciesLabel(c.species || 'Andere'),
  customSpecies: normalizeSpeciesLabel(c.customSpecies || c.custom_species || ''),
  ...
  depthM: c.depthM != null ? Number(c.depthM) : (c.depth_m != null ? Number(c.depth_m) : null),
...
async function saveCatchToSupabase(entry) {
  ...
  let payload = {
    ...
    species: normalizeSpeciesLabel(entry.species || 'Andere'),
  ...
  if (entry.depthM !== null && entry.depthM !== undefined && entry.depthM !== '') {
    payload.depth_m = Number(entry.depthM);
  }
  ...
  if (error && payload.depth_m !== undefined && String(error.message || '').includes('depth_m')) {
    const fallbackPayload = { ...payload };
    delete fallbackPayload.depth_m;
    ({ error } = await db
      .from('catches')
  ...
  if (error) {
    ...
    if (localIndex >= 0) {
      state.catches[localIndex] = {
        ...
        depthM: entry.depthM ?? state.catches[localIndex].depthM ?? null,
    ...
    if (localIndex >= 0 && typeof window.attachCatchToDuelFromCatch === 'function') {
      try { await window.attachCatchToDuelFromCatch(state.catches[localIndex]); } catch (e) { console.warn('Duell-Catch-Verknüpfung fehlgeschlagen', e); }
    }
...
if(data?.current){window.__lastWeatherData=data.current;}if(requestId!==weatherPopupRequestId)return;if(!weatherEnabled){setWeatherControlState();return}if(!data||!data.current){loadingPopup.setContent('<div class="weather-popup weather-popup--error">Open-Meteo konnte für diesen Punkt gerade keine Daten liefern.</div>');setWeatherControlState();return}loadingPopup.setContent(buildWeatherPopupHtml(data));setWeatherControlState()});map._weatherClickBound=true;}};function loadState(){const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return structuredClone(defaultData);try{const data=normalizeLegacySpeciesInObject(JSON.parse(raw));return{meta:data.meta||structuredClone(defaultData.meta),participants:Array.isArray(data.participants)?data.participants:[],catches:Array.isArray(data.catches)?data.catches:[],tournaments:Array.isArray(data.tournaments)?data.tournaments:[]}}catch{return structuredClone(defaultData)}}
...
function computeParticipantMicroAwards(data,allStats=[],allCatches=[]){
  ...
  if(nightCount>=3)add('night','🌙','Night Hunter',`${nightCount} Nachtfänge`);
...
const FISH_GAME_SPECIES={
  ...
  zander:{keys:['zander'],base:'Tiefenblick',theme:'#66e7ff',accent:'#b7a0ff',evolutions:['Dämmerling','Tiefenblick','Nachtspur','Kantenzahn','Nebeljäger','Schattenschwimmer','Dämmermeister','Tiefenräuber','Nachtkönig','Zander-Legende'],flavor:'Liest die Kanten im Halbdunkel wie eine geheime Karte.'},
  trout:{keys:['forelle','trout'],base:'Stromspringer',theme:'#ff8ab4',accent:'#66e7ff',evolutions:['Flitzer','Stromfunke','Silberfloss','Wirbelpfeil','Bachjäger','Stromspringer','Flussglanz','Strömungsmeister','Silber-Champion','Fluss-Legende'],flavor:'Elegant im Strom, hellwach bei jeder Strömungskante.'},
...
function normalizeFishGameSpecies(value){
  ...
  const cleaned=raw.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ');
  const found=Object.entries(FISH_GAME_SPECIES).find(([,cfg])=>(cfg.keys||[]).some(k=>cleaned.includes(k)));
...
function getCurrentCompanionStage(xp,speciesKey){
  const safeXp=Math.max(0,Number(xp||0));
  const stages=getCompanionEvolutionStages(speciesKey);
  return stages.reduce((best,item)=>safeXp>=item.min?item:best,stages[0]);
}
...
function computeSpeciesProgress(catches=[]){
  const map=new Map();
  (Array.isArray(catches)?catches:[]).forEach(c=>{const label=fishGameSpeciesLabelFromCatch(c);if(!label)return;const key=normalizeFishGameSpecies(label);const prev=map.get(label)||{label,key,count:0,longest:0,heaviest:0};prev.count+=1;prev.longest=Math.max(prev.longest,Number(c.lengthCm||0));prev.heaviest=Math.max(prev.heaviest,Number(c.weightKg||0));map.set(label,prev);});
  const list=[...map.values()].sort((a,b)=>b.count-a.count||b.longest-a.longest||a.label.localeCompare(b.label));
  return{list,dominant:list[0]||null,uniqueCount:list.length};
}
function fishGameIsTournamentWinner(participant){
  const id=participant?.id,name=String(participant?.name||'').replace(/[^a-zA-Z0-9 äöüÄÖÜß]/g,'').trim().toLowerCase();
  return (state?.tournaments||[]).some(t=>{
    if(!t?.finished||!t?.winner)return false;
    const ids=Array.isArray(t.winner.participantIds)?t.winner.participantIds:[];
    const names=Array.isArray(t.winner.names)?t.winner.names:(t.winner.name?[t.winner.name]:[]);
    return Boolean((id&&ids.includes(id))||names.some(n=>String(n||'').replace(/[^a-zA-Z0-9 äöüÄÖÜß]/g,'').trim().toLowerCase()===name));
  });
}
...
function computeFishGameXp(catches=[],context={}){
  ...
  const longest=sorted.reduce((m,c)=>!m||Number(c.lengthCm||0)>Number(m.lengthCm||0)?c:m,null);
  let total=0;
  sorted.forEach((c,index)=>{
    const id=fishGameCatchId(c,index);if(seenIds.has(id))return;seenIds.add(id);
    const len=Math.max(0,Number(c.lengthCm||0));
    const weight=Math.max(0,Number(c.weightKg||0));
    const speciesKey=normalizeFishGameSpecies(fishGameSpeciesLabelFromCatch(c));
    let xp=20+Math.min(120,len)+Math.min(80,weight*10);
    if(len>=80)xp+=40;
    if(len>=100)xp+=80;
    if(longest&&fishGameCatchId(longest,index)===id&&len>0)xp+=50;
    if(!seenSpecies.has(speciesKey)){seenSpecies.add(speciesKey);xp+=40;}
    if(c.tournamentId||c.tournament_id)xp+=60;
    if(fishGameIsDuelCatch(c))xp+=40;
    total+=Math.round(xp);
  });
  return total;
}
function computeCompanionEvolution(profile){
  const xp=Math.max(0,Number(profile?.xp||0));
  const stages=getCompanionEvolutionStages(profile?.speciesKey);
  let stageIndex=0;stages.forEach((item,i)=>{if(xp>=item.min)stageIndex=i;});
...
function computeParticipantGameProfile(data,allStats=[],allCatches=[]){
  ...
  const catches=Array.isArray(data?.catches)?data.catches:[];
  const speciesProgress=computeSpeciesProgress(catches);
  const speciesKey=speciesProgress.dominant?.key||'default';
  const theme=getFishGameSpeciesTheme(speciesKey);
  const xp=computeFishGameXp(catches,{participant,data,allStats,allCatches});
...
function computeParticipantPremiumBadges(profile,data,allStats=[],allCatches=[]){
  ...
  const trout=speciesCount(['trout']);if(trout>=4)add('trout-dash','Forellenflitzer',`${trout} Forellen`,'rare','trout',45);
  const nightCount=catches.filter(c=>{const h=new Date(c.timestamp||c.createdAt||0).getHours();return h>=21||h<5}).length;
  if(nightCount>=3)add('night-angler','Nachtangler',`${nightCount} Nachtfänge`,'rare','moon',55);
  if(fishGameIsTournamentWinner(profile.participant))add('tournament-crown','Turnierkrone','Turniersieg freigeschaltet','epic','crown',85);
  const duelWins=Number(profile.participant?.duelWins||profile.participant?.duel_wins||0);if(duelWins>=2)add('duel-winner','Duellsieger',`${duelWins} gewonnene Duelle`,'epic','target',82);
  const schleppCount=catches.filter(fishGameIsSchleppmeisterCatch).length;if(schleppCount>0)add('schleppmeister','Schleppmeister',`${schleppCount} Schleppmeister-Fang${schleppCount===1?'':'e'}`,'epic','route',80);
  const sniperCount=catches.filter(fishGameIsSniperCatch).length;if(sniperCount>=2)add('sniper-eye','Sniper-Auge',`${sniperCount} Zieltreffer`,'epic','target',86);
  if(profile.evolution?.stage>=10)add('legend-companion','Companion-Legende',profile.evolution.name,'legendary','crown',130);
  const rank={legendary:4,epic:3,rare:2,common:1};
  return badges.sort((a,b)=>(rank[b.rarity]-rank[a.rarity])||b.score-a.score||a.title.localeCompare(b.title));
}
...
function renderFishBadgeSvg(badge,size=64){
  const rarity=['common','rare','epic','legendary'].includes(badge?.rarity)?badge.rarity:'common',icon=badge?.icon||'fish',title=escapeHtml(badge?.title||'Badge');
...
function renderCompanionJourney(profile){
  ...
  const stages=getCompanionEvolutionStages(profile?.speciesKey);
  return `<div class="fish-evolution-journey" aria-label="Companion Journey mit ${stages.length} Evolutionen">${stages.map(item=>{const state=item.stage<ev.stage?'is-unlocked':(item.stage===ev.stage?'is-current':'is-locked');return `<span class="fish-evolution-step ${state}" title="Evolution ${item.stage}: ${escapeHtml(item.name)} · ab ${item.min} XP"><b>${item.stage}</b><small>${escapeHtml(item.name)}</small></span>`;}).join('')}</div>`;
}
function renderParticipantSeniorityPanel(participant){
  const progress=computeSeniorityProgress(participant,state?.tournaments||[]);
  const levels=[{key:'rookie',label:'Rookie',min:0},{key:'angler',label:'Angler',min:20},{key:'pro',label:'Pro',min:50},{key:'champion',label:'Champion',min:90},{key:'legend',label:'Legende',min:140}];
  const text=progress.isMax?'Maximale Seniorität erreicht':`${progress.currentLabel} → ${progress.nextLabel}`;
  return `<div class="fish-seniority-embed fish-seniority-panel"><div class="participant-game-title fish-seniority-title"><span>Seniority</span>${renderSeniorityBadge(participant)}</div><div class="fish-seniority-hero"><div><small>Rangpfad</small><strong>${escapeHtml(text)}</strong><span>${progress.isMax?'Du bist am oberen Ende der Ladder.':`Noch ${progress.remaining} Punkte bis ${escapeHtml(progress.nextLabel)}`}</span></div><div class="fish-seniority-score"><b>${Number(progress.score||0)}</b><small>Score</small></div></div>${renderSeniorityProgress(participant)}<div class="fish-seniority-ladder" aria-label="Seniority Ladder">${levels.map(level=>`<span class="${level.min<=progress.score?'is-reached':''} ${level.key===progress.key?'is-current':''}">${escapeHtml(level.label)}</span>`).join('')}</div></div>`;
}

function renderParticipantGameLayer(data,allStats=[],allCatches=[]){
  const profile=computeParticipantGameProfile(data,allStats,allCatches),badges=computeParticipantPremiumBadges(profile,data,allStats,allCatches),ev=profile.evolution;
  const participantId=String(data?.participant?.id||''),isExpanded=participantId&&FISH_GAME_BADGE_EXPANDED.has(participantId);
  const dominant=profile.dominantSpecies?.label||'Uferling',longest=profile.longest?`${Number(profile.longest.lengthCm||0).toFixed(0)} cm`:'–',heaviest=profile.heaviest?fmtKg(profile.heaviest.weightKg):'–';
  const baseBadges=badges.slice(0,6),extraBadges=badges.slice(6),rest=extraBadges.length,nextLine=ev.isMax?'Maximale Evolution erreicht':`Noch ${ev.remaining} XP bis ${ev.nextName}`;
  const toggleLabel=isExpanded?'Zusätzliche Badges ausblenden':`+${rest} weitere freigeschaltet`;
  return `<section class="participant-game-panel fish-game-panel"><article class="fish-companion-card"><div class="fish-companion-card__top"><div><p class="eyebrow">Companion Evolution</p><h3>${escapeHtml(ev.name)}</h3><span>Evolution ${ev.stage} / ${ev.totalStages}</span></div><div class="fish-companion-level"><strong>Lvl ${ev.level}</strong><small>${Number(profile.xp||0).toLocaleString('de-CH')} XP</small></div></div><div class="fish-companion-visual">${renderCompanionSvg(profile,154)}</div><div class="fish-companion-meta"><strong>${escapeHtml(profile.companionBase)}</strong><span>${escapeHtml(profile.flavor)}</span></div><div class="fish-xp-track" aria-label="${escapeHtml(nextLine)}"><div class="fish-xp-track__head"><span>${escapeHtml(nextLine)}</span><strong>${ev.percent}%</strong></div><div class="fish-xp-track__bar"><span style="width:${ev.percent}%"></span></div></div>${renderCompanionJourney(profile)}<div class="fishdex-mini"><span>FishDex: <b>${profile.uniqueSpecies}</b> Arten</span><span>Top-Art: <b>${escapeHtml(dominant)}</b></span><span>Rekord: <b>${escapeHtml(longest)}</b></span><span>Top-Gewicht: <b>${escapeHtml(heaviest)}</b></span></div></article><article class="fish-badge-collection"><div class="fish-badge-collection__head"><div><p class="eyebrow">Premium Badges</p><h3>Badge Collection</h3></div>${badges.length?`<span>${badges.length} freigeschaltet</span>`:''}</div><div class="fish-badge-grid">${baseBadges.length?baseBadges.map(renderPremiumBadgeCard).join(''):'<div class="fish-badge-empty">Deine Sammlung startet mit dem ersten Fang.<br>Trage einen Fang ein, um dein erstes Badge freizuschalten.</div>'}</div>${rest>0?`<div class="fish-badge-extra ${isExpanded?'is-open':''}" data-badge-extra-for="${escapeHtml(participantId)}"><div class="fish-badge-grid fish-badge-grid--extra">${extraBadges.map(renderPremiumBadgeCard).join('')}</div></div><button type="button" class="fish-badge-toggle" data-toggle-premium-badges="${escapeHtml(participantId)}" data-open-label="Zusätzliche Badges ausblenden" data-closed-label="+${rest} weitere freigeschaltet" aria-expanded="${isExpanded?'true':'false'}">${escapeHtml(toggleLabel)}</button>`:''}${renderParticipantSeniorityPanel(data.stats||data.participant)}</article></section>`;
}
...
function renderCumulativeCatchComparison(aData,bData){
  ...
  const aColor='var(--danger)',bColor='var(--muted)';
  ...
  const dots=(series,cls,radius)=>series.points.filter((_,i,arr)=>i===arr.length-1).map(pt=>{const x=pad.left+((pt.time-minTime)/Math.max(1,maxTime-minTime))*(width-pad.left-pad.right),y=height-pad.bottom-(pt.value/Math.max(1,maxValue))*(height-pad.top-pad.bottom);return `<circle class="${cls}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${radius}"><title>${escapeHtml(pt.label)}</title></circle>`}).join('');
  return `<section class="participant-detail-panel compare-cumulative-panel"><div class="participant-section-head"><div><p class="eyebrow">Kumulative Fangentwicklung</p><h3>Fänge über Zeit</h3></div><span class="participant-tournament-status">read-only</span></div><div class="compare-cumulative-summary"><span style="--participant-color:${aColor}"><i></i>${escapeHtml(aData.participant.name)} · ${aSeries.total}</span><span style="--participant-color:${bColor}"><i></i>${escapeHtml(bData.participant.name)} · ${bSeries.total}</span></div><div class="compare-cumulative-chart" style="--compare-a:${aColor};--compare-b:${bColor}"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Kumulative Fangentwicklung"><g class="compare-cumulative-grid">${grid}</g><path class="compare-cumulative-line compare-cumulative-line--b" d="${bPath}"/><path class="compare-cumulative-line compare-cumulative-line--a" d="${aPath}"/>${dots(bSeries,'compare-cumulative-dot compare-cumulative-dot--b',3)}${dots(aSeries,'compare-cumulative-dot compare-cumulative-dot--a',5)}<g class="compare-cumulative-axis"><line x1="${pad.left}" x2="${width-pad.right}" y1="${height-pad.bottom}" y2="${height-pad.bottom}"/><text x="${pad.left}" y="${height-14}">${startLabel}</text><text x="${width-pad.right}" y="${height-14}" text-anchor="end">${endLabel}</text></g></svg></div></section>`;
}
...
let tournamentCatchMap=null,tournamentCatchMapLayer=null,tournamentCatchMapFitBounds=null;
function getTournamentCatchTimeValue(c,index=0){const candidates=[c?.timestamp,c?.caught_at,c?.createdAt,c?.created_at].filter(Boolean);for(const value of candidates){const time=Date.parse(value);if(Number.isFinite(time))return time;}return Number.MAX_SAFE_INTEGER-index;}
function formatTournamentCatchDateTime(value){const time=Date.parse(value);if(!Number.isFinite(time))return'Zeit offen';return new Date(time).toLocaleString('de-CH',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'});}
function getTournamentCatchDisplayDate(c){const candidates=[c?.timestamp,c?.caught_at,c?.createdAt,c?.created_at].filter(Boolean);for(const value of candidates){if(Number.isFinite(Date.parse(value)))return value;}return candidates[0]||'';}
function getTournamentCatchLatLng(c){const lat=Number(c?.location?.lat ?? c?.latitude ?? c?.lat),lng=Number(c?.location?.lng ?? c?.longitude ?? c?.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;if(lat<-90||lat>90||lng<-180||lng>180)return null;return[lat,lng];}
function fallbackParticipantColor(key,index=0){const colors=['#4ad7d1','#ffb84d','#8ff0a7','#66e7ff','#ff8ab4','#b7a0ff','#f7d96b','#f58bd8','#7fd1ff','#f6a86b'];const source=String(key||index||'fishtrack');let hash=0;for(let i=0;i<source.length;i++)hash=(hash*31+source.charCodeAt(i))>>>0;return colors[hash%colors.length];}
function getTournamentParticipantColor(participant,index=0){return participant?.color||fallbackParticipantColor(participant?.id||participant?.name,index);}
function buildTournamentCatchMapData(catches){return(catches||[]).map((c,index)=>{const latlng=getTournamentCatchLatLng(c);const participant=participantById(c.participantId);return{catch:c,index,latlng,participant,color:getTournamentParticipantColor(participant,index),time:getTournamentCatchTimeValue(c,index)}}).filter(item=>item.latlng).sort((a,b)=>a.time-b.time||a.index-b.index);}
function tournamentCatchMapEmpty(message='Sobald Fänge mit Koordinaten vorliegen, erscheint hier der Fangverlauf.'){const mapEl=document.getElementById('tournamentCatchMap'),legend=document.getElementById('tournamentCatchMapLegend');if(tournamentCatchMap){tournamentCatchMap.remove();tournamentCatchMap=null;tournamentCatchMapLayer=null;tournamentCatchMapFitBounds=null;}if(mapEl){mapEl.classList.add('is-empty');mapEl.innerHTML=`<div class="tournament-catch-map-empty"><strong>Noch keine verortbaren Turnierfänge vorhanden.</strong><span>${escapeHtml(message)}</span></div>`;}if(legend)legend.innerHTML='';}
function buildTournamentCatchPopup(item,number){const c=item.catch,p=item.participant,species=speciesName(c),details=[];if(Number(c.weightKg||0)>0)details.push(fmtKg(c.weightKg));if(Number(c.lengthCm||0)>0)details.push(`${Number(c.lengthCm)} cm`);const place=c.spotLabel||c.location?.label||'';if(place)details.push(place);return`<div class="tournament-catch-popup-content"><strong style="color:${item.color}">#${number} · ${escapeHtml(species)}</strong><div>${escapeHtml(p?.avatar||'🎣')} ${escapeHtml(p?.name||'–')}</div><small>${escapeHtml(formatTournamentCatchDateTime(getTournamentCatchDisplayDate(c)))}</small>${details.length?`<small>${escapeHtml(details.join(' · '))}</small>`:''}</div>`;}
function refreshTournamentCatchMap(){if(!tournamentCatchMap)return;setTimeout(()=>{if(!tournamentCatchMap)return;tournamentCatchMap.invalidateSize();if(tournamentCatchMapFitBounds){if(tournamentCatchMapFitBounds.length===1)tournamentCatchMap.setView(tournamentCatchMapFitBounds[0],13);else tournamentCatchMap.fitBounds(tournamentCatchMapFitBounds,{padding:[28,28],maxZoom:13});}},120);}
function renderTournamentCatchMap(tournament,result){const mapEl=document.getElementById('tournamentCatchMap'),legend=document.getElementById('tournamentCatchMapLegend');if(!mapEl)return;if(typeof L==='undefined'){mapEl.classList.add('is-empty');mapEl.innerHTML='<div class="tournament-catch-map-empty"><strong>Karte nicht verfügbar.</strong><span>Leaflet konnte nicht geladen werden.</span></div>';if(legend)legend.innerHTML='';return;}const route=buildTournamentCatchMapData(result?.catches||[]);if(!route.length){tournamentCatchMapEmpty(result?.catches?.length?'Die zugeordneten Turnierfänge haben aktuell keine gültigen Koordinaten.':'Sobald Fänge mit Koordinaten vorliegen, erscheint hier der Fangverlauf.');return;}mapEl.classList.remove('is-empty');if(!tournamentCatchMap){mapEl.innerHTML='';tournamentCatchMap=L.map(mapEl,{zoomControl:false,attributionControl:true,scrollWheelZoom:false,tap:true}).setView(route[0].latlng,13);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,attribution:'&copy; OpenStreetMap'}).addTo(tournamentCatchMap);tournamentCatchMapLayer=L.layerGroup().addTo(tournamentCatchMap);L.control.zoom({position:'bottomright'}).addTo(tournamentCatchMap);}else{tournamentCatchMap.invalidateSize();tournamentCatchMapLayer.clearLayers();}const latLngs=route.map(item=>item.latlng);tournamentCatchMapFitBounds=latLngs;if(latLngs.length>1){L.polyline(latLngs,{color:'#d4dbe3',weight:3,opacity:.64,dashArray:'8 10',lineCap:'round',lineJoin:'round'}).addTo(tournamentCatchMapLayer);}route.forEach((item,i)=>{const isFirst=i===0,isLast=i===route.length-1;const marker=L.circleMarker(item.latlng,{radius:isLast?10:(isFirst?8:7),color:isLast?'#ffffff':'rgba(255,255,255,.92)',weight:isLast?3:2,fillColor:item.color,fillOpacity:.92}).addTo(tournamentCatchMapLayer);marker.bindTooltip(String(i+1),{permanent:true,direction:'center',className:'tournament-catch-map-number'});marker.bindPopup(buildTournamentCatchPopup(item,i+1),{className:'tournament-catch-popup',closeButton:true});});if(latLngs.length===1)tournamentCatchMap.setView(latLngs[0],13);else tournamentCatchMap.fitBounds(latLngs,{padding:[28,28],maxZoom:13});if(legend){const seen=new Set();legend.innerHTML=route.filter(item=>{const key=item.participant?.id||item.participant?.name||'unknown';if(seen.has(key))return false;seen.add(key);return true;}).map(item=>`<div class="legend-item"><span class="legend-color" style="background:${item.color}"></span><span>${escapeHtml(item.participant?.name||'Unbekannt')}</span></div>`).join('')+`<div class="legend-item"><span class="legend-color" style="background:#d4dbe3"></span><span>${route.length} Catch${route.length===1?'':'es'} chronologisch</span></div>`;}refreshTournamentCatchMap();}
...
function tournamentStatusBadge(tournament){
  if(!tournament)return '–';
  if(tournament.finished)return 'Beendet';
  const today=new Date().toISOString().slice(0,10);
  if(tournament.start&&today<tournament.start)return 'Geplant';
  if(tournament.end&&today>tournament.end)return 'Überfällig';
  return 'Aktiv';
}
function tournamentParticipantCount(tournament){return (tournament?.participantIds||[]).length||state.participants.length||0;}
function tournamentDateRange(tournament){return `${tournament?.start||'–'} bis ${tournament?.end||'–'}`;}
function tournamentParticipantLabel(id){const p=participantById(id);return p?`${p.avatar||'🎣'} ${p.name}`:'–';}
function tournamentLeaderGap(rows){
  if(!rows?.length)return 'Noch keine Wertung';
  if(rows.length<2)return 'Solo-Rennen';
  const gap=Number(rows[0].points||0)-Number(rows[1].points||0);
  return gap===0?'Gleichstand an der Spitze':`${gap} Punkte Vorsprung`;
}
function renderTournamentHeroKpis(tournament,result){
  const el=document.getElementById('tournamentHeroKpis');if(!el)return;
  ...
  el.innerHTML=`
    <article><span>Status</span><strong>${escapeHtml(status)}</strong><small>${escapeHtml(getTournamentRules(tournament).name)}</small></article>
    <article><span>Leader</span><strong>${escapeHtml(leader?.participant?.name||'–')}</strong><small>${escapeHtml(tournamentLeaderGap(result.rows))}</small></article>
    <article><span>Fänge</span><strong>${Number(result.catches?.length||0)}</strong><small>${tournamentParticipantCount(tournament)} Teilnehmer</small></article>
    <article><span>Letzter Moment</span><strong>${last?escapeHtml(speciesName(last)):'–'}</strong><small>${last?escapeHtml(fmtDateTime(last.timestamp)):'Noch kein Fang'}</small></article>`;
}
function renderTournamentLeaderboardRows(result){
  const leaderboard=document.getElementById('tournamentLeaderboard');if(!leaderboard)return;
  leaderboard.innerHTML='';
  if(!result.rows?.length){leaderboard.innerHTML='<div class="meta">Noch keine Teilnehmer in der Wertung.</div>';return;}
  const leaderPoints=Number(result.rows[0]?.points||0);
  result.rows.forEach((row,i)=>{
    const p=row.participant||{};
    const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':`#${i+1}`;
    const gap=i===0?'Leader':`${Math.max(0,leaderPoints-Number(row.points||0))} Punkte zurück`;
    const bonuses=row.bonuses?.slice(0,3).join(' · ')||'Nur Basiswertung';
    leaderboard.insertAdjacentHTML('beforeend',`<article class="list-card tournament-leader-card rank-${i+1}">
      <div class="tournament-rank-mark">${escapeHtml(medal)}</div>
      <div class="tournament-leader-copy">
        <div class="list-title-row"><strong>${escapeHtml(p.avatar||'🎣')} ${escapeHtml(p.name||'–')}</strong><span class="badge" style="background:${escapeHtml(p.color||'#4ad7d1')}">${Number(row.points||0)} Punkte</span></div>
        <div class="meta">${Number(row.catches||0)} Fänge · ${escapeHtml(fmtKg(row.totalWeight))} · ${escapeHtml(bonuses)}</div>
      </div>
      <div class="tournament-gap-pill">${escapeHtml(gap)}</div>
    </article>`);
  });
}
function renderTournamentLastCatch(result){
  const el=document.getElementById('tournamentLastCatch');if(!el)return;
  const last=result?.catches?.[result.catches.length-1];
  if(!last){el.innerHTML='<div class="meta">Noch kein Fang diesem Turnier zugeordnet.</div>';return;}
  const p=participantById(last.participantId);
  el.innerHTML=`<div class="tournament-last-catch-item">
    <div class="tournament-last-icon" style="background:${escapeHtml(p?.color||'#4ad7d1')}">${escapeHtml(p?.avatar||'🎣')}</div>
    <div><strong>${escapeHtml(speciesName(last))} · ${escapeHtml(last.lengthCm||'–')} cm</strong><span>${escapeHtml(p?.name||'–')} · ${escapeHtml(fmtDateTime(last.timestamp))}</span><small>${escapeHtml(last.spotLabel||last.location?.label||'Spot ohne Label')}</small></div>
  </div>`;
}
function renderTournamentCatchTimeline(result){
  const el=document.getElementById('tournamentCatchTimeline');if(!el)return;
  const catches=result?.catches||[];
  if(!catches.length){el.innerHTML='<div class="meta">Noch keine chronologischen Fangdaten vorhanden.</div>';return;}
  el.innerHTML=catches.map((c,i)=>{
    const p=participantById(c.participantId);
    return `<article class="tournament-timeline-row">
      <span class="tournament-timeline-dot" style="background:${escapeHtml(p?.color||'#4ad7d1')}">${i+1}</span>
      <div><strong>${escapeHtml(speciesName(c))} · ${escapeHtml(c.lengthCm||'–')} cm · ${escapeHtml(fmtKg(c.weightKg))}</strong><small>${escapeHtml(p?.name||'–')} · ${escapeHtml(fmtDateTime(c.timestamp))} · ${escapeHtml(c.spotLabel||c.location?.label||'Spot')}</small></div>
    </article>`;
  }).join('');
}
...
function tournamentMiniGameSnapshots(){
  const snapshots=[];
  const local=window.duelState||tournamentSafeJson('fishtrack-duel-v2');
  const sniperActive=window.sniperDuelState||tournamentSafeJson('fishtrack-duel-v2-sniper-active');
  const sniperPending=tournamentSafeJson('fishtrack-duel-v2-sniper-pending');
  const add=(s,status,source)=>{
    if(!s)return;
  ...
  if(sniperActive?.active&&sniperActive?.duelId!==local?.duelId)add({...sniperActive,mode:'sniper'},'Aktiv','sniper-active');
  if(!sniperActive?.active&&sniperPending?.sniperSpots?.length)add({...sniperPending,mode:'sniper'},'Vorbereitet','sniper-pending');
  return snapshots;
}
function renderTournamentDuelSummary(){
  const el=document.getElementById('tournamentDuelSummary');if(!el)return;
  const games=tournamentMiniGameSnapshots();
  if(!games.length){
    el.innerHTML=`<div class="tournament-empty-challenge"><strong>Keine aktive Challenge</strong><span>Mini-Games bleiben unten im Duellbereich steuerbar.</span><button type="button" class="pill secondary" data-tournament-scroll="#duelPanel">Duelle öffnen</button></div>`;
    return;
  }
  el.innerHTML=games.map(g=>`<article class="tournament-challenge-card is-${escapeHtml(g.mode)}">
    <div class="tournament-challenge-icon">${escapeHtml(g.icon)}</div>
    <div><div class="tournament-challenge-head"><strong>${escapeHtml(g.title)}</strong><span>${escapeHtml(g.status)}</span></div><small>${escapeHtml(g.players)}</small><p>${escapeHtml(g.meta)}</p><em>${escapeHtml(g.hint)}</em></div>
...
function setTournamentActiveTab(tab='overview'){
  ...
  hub.querySelectorAll('[data-tournament-tab]').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.tournamentTab===tab));
  syncTournamentMobileSections();
  if(tab==='map'&&typeof refreshTournamentCatchMap==='function')refreshTournamentCatchMap();
}
function syncTournamentMobileSections(){
  const hub=document.getElementById('tournamentHubPanel');if(!hub)return;
  const tab=hub.dataset.activeTournamentTab||'overview';
  const mobile=window.matchMedia&&window.matchMedia('(max-width: 760px)').matches;
  hub.querySelectorAll('[data-tournament-section]').forEach(section=>{
    const tags=(section.dataset.tournamentSection||'').split(/\s+/).filter(Boolean);
    section.hidden=mobile&&!tags.includes(tab);
  });
}
function bindTournamentHubInteractions(){
  if(document.body.dataset.tournamentHubBound==='1')return;
  document.body.dataset.tournamentHubBound='1';
  document.addEventListener('click',e=>{
    const tab=e.target.closest('[data-tournament-tab]');
    if(tab){e.preventDefault();setTournamentActiveTab(tab.dataset.tournamentTab||'overview');return;}
    const scroll=e.target.closest('[data-tournament-scroll]');
    if(scroll){e.preventDefault();document.querySelector(scroll.dataset.tournamentScroll)?.scrollIntoView({behavior:'smooth',block:'start'});return;}
    const collapse=e.target.closest('[data-tournament-collapse]');
    if(collapse){
      e.preventDefault();
      const target=document.getElementById(collapse.dataset.tournamentCollapse);
      if(target){
        const expanded=target.classList.toggle('is-expanded');
        target.classList.toggle('is-collapsed',!expanded);
        if(collapse.dataset.tournamentCollapse==='tournamentStory')window.tournamentStoryExpanded=expanded;
        if(collapse.dataset.tournamentCollapse==='tournamentCatchMapLegend')window.tournamentLegendExpanded=expanded;
        collapse.textContent=expanded?'Weniger':'Mehr';
        if(collapse.dataset.tournamentCollapse==='tournamentCatchMapLegend')collapse.textContent=expanded?'Legende aus':'Legende';
      }
    }
  });
  ...
  if (typeof renderDashboard === 'function') {
    ...
    renderDashboard = function(...args) {
      ...
      try {
        ...
        const list = ['Barsch','Hecht','Zander','Forelle','Karpfe','Andere'];
        list.forEach(sp => {
...
// Fangkarte tournament filter (map-only, minimal invasive)
window.mapTournamentFilter = window.mapTournamentFilter || 'overview';

function getMapVisibleCatches(sourceCatches){
  const catches = Array.isArray(sourceCatches) ? sourceCatches : (state.catches || []);
  const selected = window.mapTournamentFilter || 'overview';
  if(!selected || selected === 'overview') return catches;
  return catches.filter(c => String(c.tournamentId || '') === String(selected));
}

function getMapTournamentName(tournamentId){
  const tournament = (state.tournaments || []).find(t => String(t.id) === String(tournamentId));
  return tournament?.name || '';
}

function syncMapTournamentButton(){
  const btn = document.getElementById('map-tournament-filter-btn');
  if(!btn) return;
  const selected = window.mapTournamentFilter || 'overview';
  const hasFilter = selected !== 'overview';
  const name = hasFilter ? getMapTournamentName(selected) : '';
  btn.classList.toggle('is-active', hasFilter);
  btn.setAttribute('aria-pressed', hasFilter ? 'true' : 'false');
  btn.title = hasFilter && name ? `Aktiver Turnierfilter: ${name}` : 'Fangkarte nach Turnier filtern';
  btn.textContent = hasFilter && name ? `Turnier: ${name}` : 'Turnier filtern';
}

function refreshMapTournamentSelect(){
  const select = document.getElementById('mapTournamentSelect');
  if(!select) return;
  const tournaments = state.tournaments || [];
  const current = window.mapTournamentFilter || 'overview';
  select.innerHTML = '<option value="overview">Alle Turniere</option>' + tournaments.map(t => {
    const status = t.finished ? 'beendet' : 'offen';
    return `<option value="${escapeHtml(t.id)}">${escapeHtml(t.name)} (${status})</option>`;
  }).join('');
  if(current !== 'overview' && !tournaments.some(t => String(t.id) === String(current))){
    window.mapTournamentFilter = 'overview';
  }
  select.value = window.mapTournamentFilter || 'overview';
  syncMapTournamentButton();
}

function closeMapTournamentSelect(){
  const select = document.getElementById('mapTournamentSelect');
  const btn = document.getElementById('map-tournament-filter-btn');
...
function initMapTournamentFilterControl(){
  const btn = document.getElementById('map-tournament-filter-btn');
  const select = document.getElementById('mapTournamentSelect');
  const wrap = document.querySelector('.map-tournament-filter');
  if(!btn || !select || !wrap || btn.dataset.bound === '1') return;
  btn.dataset.bound = '1';

  btn.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    refreshMapTournamentSelect();
    const willOpen = select.classList.contains('hidden');
    select.classList.toggle('hidden', !willOpen);
  ...
  select.addEventListener('change', () => {
    window.mapTournamentFilter = select.value || 'overview';
    syncMapTournamentButton();
    closeMapTournamentSelect();
    if(typeof renderMap === 'function') renderMap();
  });

  document.addEventListener('click', e => {
    if(!wrap.contains(e.target)) closeMapTournamentSelect();
  });

  refreshMapTournamentSelect();
}

document.addEventListener('DOMContentLoaded', initMapTournamentFilterControl);
window.addEventListener('load', () => setTimeout(() => {
  initMapTournamentFilterControl();
  refreshMapTournamentSelect();
}, 50));
...
function ensureCatchDropdownFields(){
  const addMissingOption=(select,value)=>{
    if(!select||!value)return;
    const exists=[...select.options].some(option=>String(option.value||option.textContent||'').toLowerCase()===String(value).toLowerCase());
    if(!exists)select.insertAdjacentHTML('beforeend',`<option value="${value}">${value}</option>`);
  };
  ...
  addMissingOption(document.querySelector('select[name="bait"]'),'Swimbait');
  const depthField=document.querySelector('[name="depthM"]');
  if(!depthField){
    const weightField=document.querySelector('[name="weightKg"]');
    const wrapper=weightField?.closest('label');
    if(wrapper){
      wrapper.insertAdjacentHTML('afterend','<label><span>Tiefe (m) <small class="subtle-inline">(optional)</small></span><input type="number" name="depthM" min="0" step="0.1" inputmode="decimal" placeholder="optional" /></label>');
    }
  }
  ...
  const spotSelect=document.querySelector('select[name="spotLabel"]');
  addMissingOption(spotSelect,'Steg');
  addMissingOption(spotSelect,'Spundwand');
}
...
const PARTICIPANT_COACH_SPECIES=[
  {id:'pike',label:'Hecht'}, {id:'perch',label:'Barsch'}, {id:'zander',label:'Zander'}, {id:'trout',label:'Forelle'}, {id:'carp',label:'Karpfe'}, {id:'other',label:'Andere'}
];
...
function renderSpotBaitMatrix(){
  ...
  if(!container)return;
  const catches=[...state.catches];
  const optionValues=(selector)=>[...document.querySelectorAll(`${selector} option`)].map(option=>String(option.value||'').trim()).filter(Boolean);
  const uniqueOrdered=(values)=>{const seen=new Set();return values.filter(value=>{const key=String(value).toLowerCase();if(seen.has(key))return false;seen.add(key);return true;});};
  const spots=uniqueOrdered([...optionValues('#catchForm select[name="spotLabel"]'),...analyticsCountBy(catches,c=>c.spotLabel||c.location?.label||'Unbekannter Spot').map(x=>x[0])]);
  const baits=uniqueOrdered([...optionValues('#catchForm select[name="bait"]'),...analyticsCountBy(catches,c=>c.bait||'Unbekannter Köder').map(x=>x[0])]);
  if(!spots.length||!baits.length){
    container.style.removeProperty('--matrix-cols');
  ...
  const baitIcon=(bait)=>{
    ...
    if(key.includes('swimbait'))return '<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M7 35c10-11 20-16 31-14 7 1 12 5 17 10-5 5-11 9-18 10-11 2-21-1-30-6Z"/><path d="M16 34c6 4 12 5 18 2s11-3 17 1"/><path d="M47 31l10-8 1 16-11-8Z"/><circle cx="25" cy="29" r="2.4"/></svg>';
  ...
  const spotIcon=(spot)=>{
    ...
    if(key.includes('steg'))return '<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M9 39h46"/><path d="M14 31h40"/><path d="M17 25v28"/><path d="M31 25v28"/><path d="M47 25v28"/><path d="M10 50c7-3 14-3 21 0s14 3 23 0"/></svg>';
    if(key.includes('spund'))return '<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M13 13v38"/><path d="M24 13v38"/><path d="M35 13v38"/><path d="M46 13v38"/><path d="M9 51h46"/><path d="M10 38c8-3 14-3 21 0s14 3 23 0"/></svg>';
...
(function(){
  ...
  const PARALLEL_DUELS_KEY=`${KEY}-parallel-duels`;
  const PARALLEL_SELECTED_KEY=`${KEY}-parallel-selected`;
  ...
  let duelMap=null,duelRoute=null,duelMarkers=[],sniperLayer=null,sniperCatchLayer=null;
  ...
  const SNIPER_TARGET_TIERS=[
    {id:'tiny',label:'Nadelöhr',radius:18,points:100,color:'rgba(255,72,72,.28)',stroke:'#ff7070'},
    {id:'small',label:'Sharp',radius:28,points:100,color:'rgba(255,184,77,.22)',stroke:'#ffb84d'},
  ...
  function nextSniperTier(index){return SNIPER_TARGET_TIERS[Math.abs(Number(index||0))%SNIPER_TARGET_TIERS.length];}
  function isSniperMode(s){return (s?.mode||'')==='sniper';}
  function ensureSniperState(s){
    if(!s)return s;
    s.sniperSpots=Array.isArray(s.sniperSpots)?s.sniperSpots:[];
    s.sniperScore=s.sniperScore&&typeof s.sniperScore==='object'?s.sniperScore:{};
    s.sniperConfig=s.sniperConfig&&typeof s.sniperConfig==='object'?s.sniperConfig:null;
    if(isSniperMode(s)){
      const players=[s.captainId,s.opponentId].filter(Boolean);
      s.sniperConfig={spotsPerPlayer:Number(s.sniperConfig?.spotsPerPlayer||2),currentTurn:Number(s.sniperConfig?.currentTurn||0),players:s.sniperConfig?.players?.length?s.sniperConfig.players:players,locked:!!s.sniperConfig?.locked,setupDone:!!s.sniperConfig?.setupDone,setupActive:!!s.sniperConfig?.setupActive};
      s.sniperConfig.spotsPerPlayer=Math.max(1,Math.min(3,Number(s.sniperConfig.spotsPerPlayer||2)));
      s.sniperConfig.players=players.length?players:s.sniperConfig.players;
      s.sniperConfig.currentTurn=Math.max(0,Math.min(s.sniperConfig.currentTurn,Math.max(0,s.sniperConfig.players.length-1)));
  ...
  function rebuildSniperScoreFromHitSpots(s){
    s=ensureSniperState(s);
    const scores={};
    [s.captainId,s.opponentId].filter(Boolean).forEach(id=>scores[id]=0);
    (s.sniperSpots||[]).forEach(spot=>{
      if(!spot.hit||!spot.hitBy)return;
      scores[spot.hitBy]=Number(scores[spot.hitBy]||0)+Number(spot.pointsAwarded||spot.points||spot.basePoints||0);
    });
    s.sniperScore=scores;
    return s;
  }
  function attachCatchToDuel(s,catchData){
    if(!s?.active||!catchData)return s;
    s.catches=Array.isArray(s.catches)?s.catches:[];
    const catchAt=catchData.timestamp||catchData.at||new Date().toISOString();
    const catchEntry={...catchData,at:catchAt};
    const existingIdx=s.catches.findIndex(c=>c.id&&catchEntry.id&&c.id===catchEntry.id);
    if(existingIdx>=0)s.catches[existingIdx]={...s.catches[existingIdx],...catchEntry};
    else s.catches.push(catchEntry);
      ...
      const point=Number.isFinite(Number(catchData.location?.lat))&&Number.isFinite(Number(catchData.location?.lng))
        ...
        : (Number.isFinite(Number(catchData.lat))&&Number.isFinite(Number(catchData.lng))
          ...
          : (Number.isFinite(Number(catchData.latitude))&&Number.isFinite(Number(catchData.longitude))
            ? {lat:Number(catchData.latitude),lng:Number(catchData.longitude)}
            : null));
      const target=catchData.participantId||catchData.participant_id||catchData.user_id||catchData.userId||catchData.fisherId||catchData.fisher_id||null;
      if(target&&s.sniperScore[target]==null)s.sniperScore[target]=0;
      let catchPatch={sniperHit:false,sniperPoints:0};
      if(point){
        let best=null;
        (s.sniperSpots||[]).forEach(spot=>{
          const dist=distanceMeters(spot,point),radius=Number(spot.radius||spot.baseRadiusM||50);
          if(dist<=radius&&(!best||dist<best.dist))best={spot,dist};
        });
        if(best&&!best.spot.hit&&target){
          const points=Number(best.spot.points||best.spot.basePoints||100);
          best.spot.hit=true;
          best.spot.hitBy=target;
          best.spot.hitCatchId=catchData.id||null;
          best.spot.hitDistanceM=Math.round(best.dist);
          best.spot.pointsAwarded=points;
          best.spot.hitTier='Hit';
          s.sniperScore[target]=Number(s.sniperScore[target]||0)+points;
          catchPatch={sniperHit:true,sniperSpotId:best.spot.id||null,sniperDistanceM:Math.round(best.dist),sniperPoints:points};
...
(catchData){
    ...
    const participantId=catchData.participantId
      || catchData.participant_id
      || catchData.user_id
      || catchData.userId
      || catchData.fisherId
      || catchData.fisher_id
    ...
    const allowed=[sniper.captainId,sniper.opponentId].filter(Boolean);

    // Spot Sniper Elite wertet nur eindeutig zuordenbare Fänge von Kapitän/Herausforderer.
    if(!participantId||!allowed.includes(participantId))return;

    const catchAt=catchData.timestamp
      || catchData.at
      || catchData.created_at
      || catchData.createdAt
      || new Date().toISOString();

    const catchMs=new Date(catchAt).getTime();
    const startMs=new Date(sniper.startedAt||sniper.startTimestamp||0).getTime();
    const endMs=sniper.endedAt?new Date(sniper.endedAt).getTime():Infinity;

    if(!Number.isFinite(catchMs))return;
    if(Number.isFinite(startMs)&&catchMs<startMs)return;
    if(Number.isFinite(endMs)&&catchMs>endMs)return;

    const lat=Number(catchData.location?.lat??catchData.lat??catchData.latitude);
    const lng=Number(catchData.location?.lng??catchData.lng??catchData.longitude);

    if(!Number.isFinite(lat)||!Number.isFinite(lng))return;

    const catchId=catchData.id||catchData.catch_id||catchData.localId||null;

    if(catchId&&Array.isArray(sniper.catches)&&sniper.catches.some(c=>String(c.id||c.catch_id||c.localId||'')===String(catchId))){
      return;
    }

    const beforeHits=(sniper.sniperSpots||[]).filter(spot=>spot.hit).length;

    sniper=attachCatchToDuel(sniper,{
      ...catchData,
      id:catchId||catchData.id,
      participantId,
      at:catchAt,
      timestamp:catchAt,
      location:{
    ...
    const afterHits=(sniper.sniperSpots||[]).filter(spot=>spot.hit).length;
    ...
    try{
      ...
      if(sniper?.active){
        await syncRemoteParticipants(sniper);
      }
    }catch(e){
      console.warn('Sniper remote catch sync failed',e);
    }

    updateDuelUi();

    requestAnimationFrame(()=>{
      renderSniperSectionMap(getActiveSniperDuel());
    });

    if(typeof loadDuelLeaderboard==='function'){
      loadDuelLeaderboard();
    }
  }
  ...
  function getOpenDuelsForCatch(catchData,exclude=[]){
    const participantId=catchParticipantId(catchData);
    if(!participantId)return [];
    return getActiveDuels().filter(duel=>{
      if(!duel?.active)return false;
      if(exclude.some(item=>sameParallelDuel(item,duel)))return false;
      const allowed=[duel.captainId,duel.opponentId].filter(Boolean);
      if(!allowed.includes(participantId))return false;
      if(!isCatchInsideDuelWindow(duel,catchData))return false;
      if(hasDuelProcessedCatch(duel,catchData))return false;
      return true;
    });
  }
  function persistParallelSyncedDuel(duel){
    if(!duel)return;
    duel.lastActivityAt=new Date().toISOString();
    duel.updatedAt=duel.lastActivityAt;
    const current=getDuelState();
    if(sameParallelDuel(current,duel)){
      saveDuelState(duel);
    }else{
      upsertParallelDuel(duel,{select:false,unreadActivity:true});
    }
  }
  async function syncCatchIntoDuel(duel,catchData){
    duel=normalizeParallelDuel(duel);
    if(!duel?.active||!catchData)return null;
    const participantId=catchParticipantId(catchData);
    const allowed=[duel.captainId,duel.opponentId].filter(Boolean);
    if(!participantId||!allowed.includes(participantId)||!isCatchInsideDuelWindow(duel,catchData)||hasDuelProcessedCatch(duel,catchData))return null;
    try{
      if(isSniperMode(duel)){
        duel=ensureSniperState(duel);
        const lat=Number(catchData.location?.lat??catchData.lat??catchData.latitude);
        const lng=Number(catchData.location?.lng??catchData.lng??catchData.longitude);
        if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;
        const beforeHits=(duel.sniperSpots||[]).filter(spot=>spot.hit).length;
        const catchEvent=buildSyncedDuelCatch(catchData,participantId,fishTileForCatch(catchData),{duelSynced:true,sniperAttached:true,duelType:'sniper',scoringSource:'catches'});
        catchEvent.location={...(catchData.location||{}),lat,lng};
        duel=attachCatchToDuel(duel,catchEvent);
        const afterHits=(duel.sniperSpots||[]).filter(spot=>spot.hit).length;
        duel.lastTalk=afterHits>beforeHits?duel.lastTalk:`🎯 Sniper: ${participantName(participantId)} ausserhalb der Zielscheibe – Fang wurde dennoch dedupliziert registriert.`;
        persistParallelSyncedDuel(duel);
        await addRemoteEvent(duel,'catch',participantId,{...catchEvent,sniperAttached:afterHits>beforeHits});
      ...
      if(duel.mode==='feed'){
        ...
        duel.lastTalk=`${participantName(participantId)} füttert Feed your Fish parallel aus „Fänge“ mit ${fish.species}. +${Number(fish.points||0)}P.`;
        persistParallelSyncedDuel(duel);
        await addRemoteEvent(duel,'catch',participantId,catchEvent);
        await syncRemoteParticipants(duel);
        return duel;
      }
      if(duel.mode==='trolling'){
        const fish=fishTileForCatch(catchData);
        ...
        const basePoints=5;
        const catchEvent=buildSyncedDuelCatch(catchData,participantId,{species:fish.species,points:basePoints},{duelSynced:true,schleppmeisterSynced:true,catchId,basePoints,speedBonusPoints,speedBonusParticipantId:duel.captainId||null,avgSpeedKmh:avgSpeedKmh!=null&&Number.isFinite(Number(avgSpeedKmh))?Number(Number(avgSpeedKmh).toFixed(2)):null,speedBonusApplied:!!speedBonusPoints,scoringSource:'catches'});
        duel.score=duel.score||{};
        ...
        duel=attachCatchToDuel(duel,catchEvent);
        duel.lastTalk=`${participantName(participantId)} bringt einen parallelen Schleppmeister-Fang. +${basePoints}P${speedBonusPoints?` · Kapitän-Speed-Bonus +${speedBonusPoints}P`:''}`;
        persistParallelSyncedDuel(duel);
        await addRemoteEvent(duel,'catch',participantId,catchEvent);
      ...
      console.warn('Parallel-Duell Catch-Sync fehlgeschlagen',e);
      return null;
    }
  }
  async function syncCatchIntoAllOpenDuels(catchEntry,exclude=[]){
    const eligible=getOpenDuelsForCatch(catchEntry,exclude);
    for(const duel of eligible){await syncCatchIntoDuel(duel,catchEntry);}
    if(eligible.length)renderParallelDuelsHub(getDuelState());
    return eligible.length;
  }
  window.syncCatchIntoAllOpenDuels=syncCatchIntoAllOpenDuels;
  async function attachCatchToDuelFromCatch(catchData){
    ...
    await syncCatchIntoAllOpenDuels(catchData,[duel,getActiveSniperDuel()].filter(Boolean));
  ...
  function sniperSummaryText(s){
    ...
    const cfg=ensureSniperState(s).sniperConfig;
  ...
  function renderSniperHud(){
    ...
    const show=isSniperMode(s);
    if(!hud){hud=document.createElement('div');hud.id='sniperEliteHud';hud.className='sniper-elite-hud hidden';const mapCard=document.querySelector('.duel-map-card');(mapCard||panel).appendChild(hud);}
    if(!show){hud.classList.add('hidden');return;}
    const cfg=s.sniperConfig||{};
    const pid=currentSniperPlayerId(s);
    const count=sniperSpotsForPlayer(s,pid).length;
    const rows=(cfg.players||[s.captainId,s.opponentId]).filter(Boolean).map(id=>`<div><span>${escapeHtml(participantAvatar(id))} ${escapeHtml(participantName(id))}</span><b>${sniperSpotsForPlayer(s,id).length}/${cfg.spotsPerPlayer||2} Spots · ${Number(s.sniperScore?.[id]||0)} P</b></div>`).join('');
    hud.innerHTML=`<strong>${escapeHtml(sniperSummaryText(s))}</strong><small>${cfg?.setupActive&&!cfg.locked?'Tippe auf die Karte, um Ziele zu setzen. Unterschiedliche Ringgrössen entstehen automatisch.':'Trefferzonen: 100 / 50 / 20 / 10 Punkte.'}</small><div class="sniper-hud-rows">${rows}</div>${cfg?.setupActive&&!cfg.locked?`<button type="button" class="pill primary" id="sniperTurnDoneBtn" ${count<Number(cfg.spotsPerPlayer||2)?'disabled':''}>${count<Number(cfg.spotsPerPlayer||2)?'Noch Spots setzen':'Spots locken'}</button>`:''}`;
    hud.classList.remove('hidden');
  }
  function renderPendingSniperUi(){
    ...
    card.classList.toggle('is-sniper-finished',hasFinished&&!hasPending&&!hasLive);
    [startBtn,editBtn,clearBtn].forEach(btn=>{if(btn)btn.classList.toggle('hidden',!hasPending);});
    [endBtn,cancelBtn].forEach(btn=>{if(btn)btn.classList.toggle('hidden',!hasLive);});
    if(!hasLive){
      delete grid.dataset.sniperLiveKey;
      destroySniperSectionMap();
    }
    if(hasLive){
      const ids=[active.captainId,active.opponentId].filter(Boolean);
      const total=(active.sniperSpots||[]).length;
      const hits=(active.sniperSpots||[]).filter(spot=>spot.hit).length;
      const rows=ids.map(id=>{
        const spots=sniperSpotsForPlayer(active,id),hitCount=spots.filter(spot=>spot.hit).length;
        return `<article class="sniper-live-score"><div><strong>${escapeHtml(participantAvatar(id))} ${escapeHtml(participantName(id))}</strong><small>${hitCount}/${spots.length} Ziele getroffen · Catch-im-Spot zählt</small></div><b>${Number(active.sniperScore?.[id]||0)}P</b></article>`;
      }).join('');
      const progressWidth=total?Math.round((hits/total)*100):0;
      const liveKey=`${active.duelId||'sniper-local'}-${active.startedAt||''}`;
      if(grid.dataset.sniperLiveKey!==liveKey||!document.getElementById('sniperSectionMap')){
        destroySniperSectionMap();
        grid.dataset.sniperLiveKey=liveKey;
        grid.innerHTML=`
          ...
          <div class="sniper-target-progress" id="sniperTargetProgress">
            <div><b>${hits}/${total}</b><span>Ziele getroffen</span></div>
            <i style="width:${progressWidth}%"></i>
          </div>
          <div id="sniperLiveScoreRows" class="sniper-live-score-grid">${rows}</div>
        `;
      }else{
        const timer=grid.querySelector('#sniperLiveTimer');
        if(timer)timer.textContent=`${formatTime(remainingMs(active))} Restzeit`;
        const progress=grid.querySelector('#sniperTargetProgress');
        if(progress){
          const b=progress.querySelector('b');
          const bar=progress.querySelector('i');
          if(b)b.textContent=`${hits}/${total}`;
    ...
    if(hasPending){
      const cfg=pending.sniperConfig||{};
      const ids=(cfg.players||[pending.captainId,pending.opponentId]).filter(Boolean);
      const duration=Number(pending.durationMin||60);
      const durationText=duration>=60?`${duration/60===1?'1 Stunde':(duration/60)+' Stunden'}`:`${duration} Minuten`;
      const created=pending.updatedAt||pending.createdAt;
      const meta=created&&typeof fmtDateTime==='function'?fmtDateTime(created):'bereit';
      const rows=ids.map(id=>{
        const spots=sniperSpotsForPlayer(pending,id);
        return `<article class="sniper-spot-mini-card"><div><strong>${escapeHtml(participantAvatar(id))} ${escapeHtml(participantName(id))}</strong><small>${spots.length}/${Number(cfg.spotsPerPlayer||2)} Spots · ${durationText}</small></div><b>${spots.reduce((sum,spot)=>sum+Number(spot.points||spot.basePoints||0),0)}P max.</b></article>`;
      }).join('');
      grid.innerHTML=`<div class="sniper-elite-summary"><span>🔒 Setup gelockt</span><strong>${(pending.sniperSpots||[]).length} Zielscheiben</strong><small>${escapeHtml(meta)} · Start erfolgt bewusst über diese Section. Nur Treffer färben Zielscheiben grün. Keine Route, keine Catch-Marker.</small></div>${rows}`;
      return;
    }
    if(hasFinished){
  ...
  async function startPendingSniperDuel(){
    ...
    if(!pending||!pending.sniperSpots?.length){alert('Kein vorbereitetes Spot-Sniper-Setup vorhanden.');return;}
    if(hasActiveSniperDuel()){alert('Es läuft bereits ein Spot-Sniper-Duell.');return;}
    const startIso=new Date().toISOString();
    let s=ensureSniperState({
      ...defaultDuelState(),
      active:true,
      startedAt:startIso,
      startTimestamp:startIso,
      endedAt:null,
      durationMin:Number(pending.durationMin||60),
      captainId:pending.captainId,
      opponentId:pending.opponentId,
      mode:'sniper',
      score:{[pending.captainId]:0,[pending.opponentId]:0},
      sniperScore:{[pending.captainId]:0,[pending.opponentId]:0},
      sniperSpots:(pending.sniperSpots||[]).map(spot=>({...spot,hit:false,hitBy:null,hitCatchId:null,hitDistanceM:null,pointsAwarded:0,hitTier:null})),
  ...
  function editPendingSniperSetup(){
    const pending=getPendingSniperDuel();
    if(!pending||!pending.sniperSpots?.length){alert('Kein vorbereitetes Spot-Sniper-Setup vorhanden.');return;}
    const current=getDuelState();
    if(current.active){alert('Während eines laufenden Duells kann das Setup nicht bearbeitet werden.');return;}
    const cfg={...(pending.sniperConfig||{}),locked:false,setupDone:false,setupActive:true,currentTurn:0};
    const s=ensureSniperState({...defaultDuelState(),...pending,duelId:null,active:false,startedAt:null,startTimestamp:null,endedAt:null,route:[],catches:[],mode:'sniper',sniperConfig:cfg,lastTalk:'🎯 Spot Sniper Elite Setup wird bearbeitet. Tippe auf die Karte, um Spots zu ersetzen.'});
    clearPendingSniperDuel();
    saveDuelState(s);
  ...
  function openSniperSetupModal(){
    ...
    const pid=currentSniperPlayerId(s),count=sniperSpotsForPlayer(s,pid).length,need=Number(s.sniperConfig.spotsPerPlayer||2);
    let modal=document.getElementById('sniperSetupModal');
    if(!modal){modal=document.createElement('div');modal.id='sniperSetupModal';modal.className='sniper-setup-modal hidden';modal.innerHTML='<div class="sniper-setup-card glass-card"><div id="sniperSetupBody"></div></div>';document.body.appendChild(modal);}
    const body=modal.querySelector('#sniperSetupBody');
    if(body)body.innerHTML=`<p class="eyebrow">Spot Sniper Elite</p><h2>🎯 ${escapeHtml(participantName(pid))} ist dran</h2><p>${escapeHtml(participantName(pid))} darf <b>${need} Spot${need===1?'':'s'}</b> auf der Karte setzen. Danach wird gelockt und der nächste Spieler kommt dran.</p><div class="sniper-setup-meter"><span style="width:${Math.min(100,(count/need)*100)}%"></span></div><small>${count}/${need} Spots gesetzt · transparente Ringe: 100 / 50 / 20 / 10 Punkte</small><div class="sniper-setup-actions"><button type="button" class="pill" id="sniperSetupCloseBtn">Auf Karte setzen</button><button type="button" class="pill primary" id="sniperTurnDoneBtn" ${count<need?'disabled':''}>${count<need?'Noch Spots setzen':'Spots locken'}</button></div>`;
    modal.classList.remove('hidden');document.body.classList.add('sniper-setup-open');
  }
  function closeSniperSetupModal(){const modal=document.getElementById('sniperSetupModal');if(modal)modal.classList.add('hidden');document.body.classList.remove('sniper-setup-open');}
  function sniperDifficultyOptionsHtml(selected='hard'){return Object.values(SNIPER_DIFFICULTIES).map(d=>`<option value="${escapeHtml(d.id)}" ${d.id===selected?'selected':''}>${escapeHtml(d.label)} · ${d.radius} m · ${d.points}P</option>`).join('');}
  function chooseSniperDifficulty(defaultId='hard'){return new Promise(resolve=>{let modal=document.getElementById('sniperDifficultyModal');if(!modal){modal=document.createElement('div');modal.id='sniperDifficultyModal';modal.className='sniper-setup-modal hidden';modal.innerHTML='<div class="sniper-setup-card glass-card"><div id="sniperDifficultyBody"></div></div>';document.body.appendChild(modal);}const body=modal.querySelector('#sniperDifficultyBody');body.innerHTML=`<p class="eyebrow">Spot Sniper Elite</p><h2>🎚️ Schwierigkeit wählen</h2><p>Wähle den Schwierigkeitsgrad für diesen Spot.</p><label class="sniper-difficulty-field"><span>Schwierigkeitsgrad</span><select id="sniperDifficultySelect">${sniperDifficultyOptionsHtml(defaultId)}</select></label><div class="sniper-setup-actions"><button type="button" class="pill" id="sniperDifficultyCancelBtn">Abbrechen</button><button type="button" class="pill primary" id="sniperDifficultySaveBtn">Spot übernehmen</button></div>`;const finish=value=>{modal.classList.add('hidden');document.body.classList.remove('sniper-setup-open');resolve(value);};body.querySelector('#sniperDifficultyCancelBtn').onclick=()=>finish(null);body.querySelector('#sniperDifficultySaveBtn').onclick=()=>finish(body.querySelector('#sniperDifficultySelect')?.value||defaultId);modal.classList.remove('hidden');document.body.classList.add('sniper-setup-open');});}
  async function finishSniperTurn(){
    let s=ensureSniperState(getDuelState());if(!isSniperMode(s)||!s.sniperConfig?.setupActive||s.sniperConfig.locked)return;
    const pid=currentSniperPlayerId(s),need=Number(s.sniperConfig.spotsPerPlayer||2);
    if(sniperSpotsForPlayer(s,pid).length<need){openSniperSetupModal();return;}
    if(s.sniperConfig.currentTurn < s.sniperConfig.players.length-1){s.sniperConfig.currentTurn+=1;s.lastTalk=`${participantName(pid)} hat gelockt. ${participantName(currentSniperPlayerId(s))} setzt jetzt die nächsten Elite-Spots.`;saveDuelState(s);renderSniperSpots();renderSniperHud();openSniperSetupModal();return;}
    await lockSniperSetupAndStart();
  }
  async function lockSniperSetupAndStart(){
    let s=ensureSniperState(getDuelState());if(!isSniperMode(s))return;
    const needed=Number(s.sniperConfig?.spotsPerPlayer||2);
    const players=(s.sniperConfig?.players||[s.captainId,s.opponentId]).filter(Boolean);
    const complete=players.every(id=>sniperSpotsForPlayer(s,id).length>=needed);
  ...
  async function addSniperSpotAt(latlng){
    let s=ensureSniperState(getDuelState());if(!isSniperMode(s)||!s.sniperConfig?.setupActive||s.sniperConfig.locked)return;
    ...
    const chosen=await chooseSniperDifficulty(existing[existing.length-1]?.difficulty||'hard');
    s=ensureSniperState(getDuelState());if(!chosen||!isSniperMode(s)||!s.sniperConfig?.setupActive||s.sniperConfig.locked)return;
    const difficulty=sniperDifficultyById(chosen);
  ...
  function renderSniperSpots(){
    ...
    (s.sniperSpots||[]).forEach((spot,i)=>{
      ...
      const rings=[
        ...
        {...ringBase[2],color:'rgba(255,226,92,.13)',stroke:'rgba(255,226,92,.46)'},
        {...ringBase[1],color:'rgba(255,184,77,.17)',stroke:'rgba(255,184,77,.52)'},
        {...ringBase[0],color:'rgba(255,72,72,.24)',stroke:'rgba(255,112,112,.72)'}
      ];
  ...
  function buildSniperPreviewSvg(s){
    const spots=Array.isArray(s?.sniperSpots)
      ? s.sniperSpots
      : Array.isArray(s?.result?.sniper_spots)
        ? s.result.sniper_spots
        : Array.isArray(s?.fish_state?.sniper_spots)
          ? s.fish_state.sniper_spots
          : [];
    ...
    if(!spots.length){
      return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}">
        <rect width="100%" height="100%" rx="28" fill="#08111b"/>
        <text x="50%" y="47%" text-anchor="middle" fill="#eef6ff" font-family="Arial" font-size="22" font-weight="800">🎯 Spot Sniper Elite</text>
        <text x="50%" y="58%" text-anchor="middle" fill="#9db0c3" font-family="Arial" font-size="16">Keine Zielscheiben gespeichert</text>
      </svg>`;
    }
    ...
    const valid=spots
      .map(spot=>({
        ...
        lat:Number(spot.lat),
        lng:Number(spot.lng),
        radius:Number(spot.radiusM||spot.radius||spot.baseRadiusM||30)
      }))
    ...
    if(!valid.length){
      return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}">
        <rect width="100%" height="100%" rx="28" fill="#08111b"/>
        <text x="50%" y="50%" text-anchor="middle" fill="#9db0c3" font-family="Arial" font-size="18">🎯 Zielscheiben ohne Koordinaten</text>
      </svg>`;
    }
    ...
    const rings=valid.map((spot,index)=>{
      const [x,y]=xy(spot);
      const hit=!!spot.hit;
      const color=hit?'#35f28b':'#4ad7d1';
      const label=hit?'HIT':String(index+1);

      return `
        <g filter="url(#glow)">
          <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="26" fill="${hit?'rgba(53,242,139,.18)':'rgba(74,215,209,.12)'}" stroke="${color}" stroke-width="3"/>
          <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="16" fill="transparent" stroke="${hit?'#8ff0a7':'#ffb84d'}" stroke-width="2" opacity=".9"/>
          <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="6" fill="${color}" stroke="#fff" stroke-width="2"/>
          <text x="${x.toFixed(1)}" y="${(y+42).toFixed(1)}" text-anchor="middle" fill="${color}" font-family="Arial" font-size="12" font-weight="800">${label}</text>
        </g>`;
    }).join('');
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}">
      <defs>
        <radialGradient id="sniperBg" cx="18%" cy="0%" r="95%">
          <stop offset="0" stop-color="#14394a"/>
          <stop offset=".55" stop-color="#0b1824"/>
      ...
      <rect width="100%" height="100%" rx="28" fill="url(#sniperBg)"/>
      <path d="M${pad} ${h-pad} C${w*.32} ${h*.62} ${w*.62} ${h*.28} ${w-pad} ${pad}" fill="none" stroke="rgba(255,255,255,.055)" stroke-width="20" stroke-linecap="round"/>
      ${rings}
      <text x="${pad}" y="30" fill="#eef6ff" font-family="Arial" font-size="16" font-weight="900">🎯 Spot Sniper Elite</text>
  ...
  async function createRemoteDuel(s,options={}){
  ...
  async function deleteRemoteDuelCascadeSafe(duelId){
    if(!db||!duelId||duelId==='local')return false;
    let ok=true;
    for(const table of ['duel_events','duel_tracks','duel_participants','duels']){
      try{
        const query=table==='duels'?db.from(table).delete().eq('id',duelId):db.from(table).delete().eq('duel_id',duelId);
        const {error}=await query;
        if(error){ok=false;console.warn(`Duell Delete fehlgeschlagen in ${table}`,error);}
      }catch(e){ok=false;console.warn(`Duell Delete Exception in ${table}`,e);}
    }
    return ok;
  }
  async function deleteDuelFromLeaderboard(duelId){
    if(!duelId)return;
    if(!confirm('Dieses Duell wirklich aus der Rangliste löschen? Fänge bleiben erhalten.'))return;
  ...
  function buildDuelArenaStats(entries){
    const stats=new Map(),modeCounts={};
    entries.filter(d=>d.status!=='cancelled').forEach(d=>{
      const mode=duelModeInfo(d).name;modeCounts[mode]=Number(modeCounts[mode]||0)+1;
      const parts=participantRowsForDuel(d);const winner=parts[0]?.participant_id||d.result?.winner_id||null;
      parts.forEach(p=>{
        const id=p.participant_id||p.id;if(!id)return;
        const item=stats.get(id)||{id,name:p.display_name||participantName(id),played:0,wins:0,losses:0,points:0,last:d.created_at||''};
        item.played+=1;item.points+=Number(p.score||0)+Number(p.bonus_score||0);
        if(winner&&winner===id)item.wins+=1;else item.losses+=1;
        if((d.created_at||'')>item.last)item.last=d.created_at||item.last;
        stats.set(id,item);
      });
    });
    const players=[...stats.values()].sort((a,b)=>b.wins-a.wins||b.points-a.points||b.played-a.played);
    const topMode=Object.entries(modeCounts).sort((a,b)=>b[1]-a[1])[0]?.[0]||'–';
    return {players,topMode};
  }
  function renderDuelLeaderboard(){
    ...
    const arena=buildDuelArenaStats(entries);
    const podium=arena.players.slice(0,3).map((p,i)=>`<article class="duel-podium-card rank-${i+1}"><img class="duel-rank-fish" src="Transformation/Stufe_${duelFishStageForRank(i,Math.max(3,arena.players.length))}.png" alt="Rang ${i+1}" loading="lazy"><div><span>#${i+1}</span><strong>${escapeHtml(p.name)}</strong><small>${p.wins} Siege · ${p.points}P · ${p.played} Duelle</small></div></article>`).join('')||'<div class="meta">Noch kein Podium.</div>';
    ...
    const historyHtml=entries.map((d,entryIndex)=>{
      ...
      const parts=participantRowsForDuel(d);
      ...
      return `<article class="duel-history-entry duel-arena-entry is-${escapeHtml(mode.key)}">
        <div class="duel-history-copy">
          <div class="duel-card-topline"><span class="duel-mode-badge">${escapeHtml(mode.label)}</span><span class="duel-status-badge ${d.status==='active'?'is-live':d.status==='cancelled'?'is-cancelled':'is-finished'}">${escapeHtml(status)}</span></div>
          <strong>${escapeHtml(result.winner_name||parts[0]?.display_name||'Duell')}</strong>
          <small>${escapeHtml(meta)}</small>
          ${rows || '<div class="meta">Keine Teilnehmerdaten.</div>'}
          <div class="duel-history-actions">${deletable?`<button type="button" class="duel-delete-btn" data-delete-duel-id="${escapeHtml(d.id)}">🗑 Löschen</button>`:''}</div>
        </div>
        <div class="duel-photo-container">${image}<img class="duel-rank-fish duel-rank-fish-corner" src="${rankFish}" alt="Ranking-Deko" loading="lazy"></div>
      </article>`;
    }).join('');
  ...
  function sameParallelDuel(a,b){
    if(!a||!b)return false;
    const aid=a.parallelId||a.duelId||a.id;
    const bid=b.parallelId||b.duelId||b.id;
    if(aid&&bid&&String(aid)===String(bid))return true;
    if(a.duelId&&b.duelId&&String(a.duelId)===String(b.duelId))return true;
    const ast=a.startedAt||a.startTimestamp||'';
    const bst=b.startedAt||b.startTimestamp||'';
    return !!(ast&&bst&&String(ast)===String(bst)&&String(a.mode||'')===String(b.mode||'')&&String(a.captainId||'')===String(b.captainId||'')&&String(a.opponentId||'')===String(b.opponentId||''));
  }
  ...
  function isParallelDuelOpen(s){
    if(!s)return false;
    return !!(s.active||(isSniperMode(s)&&s.sniperConfig?.setupActive)||(isSniperMode(s)&&s.sniperConfig?.setupDone&&!s.endedAt)||(isDartMode(s)&&s.dartConfig?.setupActive)||(isDartMode(s)&&s.dartConfig?.setupDone&&!s.endedAt));
  }
  function isParallelDuelTrackable(s){
    if(!s)return false;
  ...
  function dedupeParallelDuels(list){
    const out=[];
    (list||[]).map(normalizeParallelDuel).filter(Boolean).forEach(item=>{
  ...
  function upsertParallelDuel(s,{select=false,unreadActivity=false}={}){
    ...
    if(select)localStorage.setItem(PARALLEL_SELECTED_KEY,item.parallelId);
    return next.find(d=>sameParallelDuel(d,item))||item;
  }
  function removeParallelDuel(s){
    if(!s)return;
    writeParallelDuelStore(readParallelDuelStore().filter(item=>!sameParallelDuel(item,s)));
  }
  function parallelDuelSort(a,b){
    const au=a.unreadActivity?1:0,bu=b.unreadActivity?1:0;if(au!==bu)return bu-au;
    const ao=isParallelDuelOpen(a)?1:0,bo=isParallelDuelOpen(b)?1:0;if(ao!==bo)return bo-ao;
    const ar=remainingMs(a),br=remainingMs(b);if(a.active&&b.active&&ar!==br)return ar-br;
    const as=Math.abs(totalScore(a,a.captainId)-totalScore(a,a.opponentId));
    const bs=Math.abs(totalScore(b,b.captainId)-totalScore(b,b.opponentId));
    if(a.active&&b.active&&as!==bs)return as-bs;
    return new Date(b.lastActivityAt||b.updatedAt||b.startedAt||0)-new Date(a.lastActivityAt||a.updatedAt||a.startedAt||0);
  }
  function getActiveDuels(){
    const current=normalizeParallelDuel(getDuelState());
  ...
  function getDuelScoreSummary(duel){
    const a=duel?.captainId,b=duel?.opponentId;
    if(!a&&!b)return 'Score offen';
    return `${totalScore(duel,a)} : ${totalScore(duel,b)}`;
  }
  function getDuelLeaderSummary(duel){
    const ids=[duel?.captainId,duel?.opponentId].filter(Boolean);
    if(ids.length<2)return 'Teilnehmer offen';
    const scores=ids.map(id=>({id,score:totalScore(duel,id)})).sort((a,b)=>b.score-a.score);
    if(scores[0].score===scores[1].score)return 'Gleichstand';
    return `${participantName(scores[0].id)} führt`;
  }
  function getDuelLastActivity(duel){
    const last=latestDuelCatch(duel);
    if(last)return `${participantName(catchParticipantId(last)||last.participantId)} · ${last.species||speciesName(last)||'Fang'} · ${fmtDateTime(catchTimestampValue(last))}`;
    if(duel?.lastTalk)return duel.lastTalk;
    return 'Noch keine Aktivität';
  }
  function getDuelStatusBadge(duel){
    if(duel?.unreadActivity)return 'Neu';
    if(duel?.active&&remainingMs(duel)<=5*60*1000)return 'Knapp';
    if(duel?.active)return 'Aktiv';
    if((isSniperMode(duel)&&duel.sniperConfig?.setupActive)||(isDartMode(duel)&&duel.dartConfig?.setupActive))return 'Setup offen';
    if((isSniperMode(duel)&&duel.sniperConfig?.setupDone)||(isDartMode(duel)&&duel.dartConfig?.setupDone))return 'Startbereit';
    if(duel?.endedAt)return 'Abgeschlossen';
    return 'Bereit';
  }
  ...
  function renderParallelDuelsHub(selectedState){
    ...
    const cards=duels.map(duel=>{
      ensureParallelDuelId(duel);
      const active=sameParallelDuel(duel,selected);
      const info=duelModeCopy(duel.mode||'trolling');
      const status=getDuelStatusBadge(duel);
      const last=latestDuelCatch(duel);
      const lastText=last?`${last.dartSynced?`🎯 ${last.dartRingLabel||'Dart'}`:(last.feedSynced?'🐟 Feed':(last.schleppmeisterSynced?'🚤 Sync':'🎣 Fang'))} · ${fmtDateTime(catchTimestampValue(last))}`:getDuelLastActivity(duel);
      return `<button type="button" class="parallel-duel-card ${active?'is-active':''} ${duel.unreadActivity&&!active?'has-new':''}" data-parallel-duel-key="${escapeHtml(duel.parallelId)}">
        <span class="parallel-duel-icon">${escapeHtml(info.icon)}</span>
        <span class="parallel-duel-main"><b>${escapeHtml(info.name)}</b><small>${escapeHtml(participantName(duel.captainId))} vs ${escapeHtml(participantName(duel.opponentId))}</small><em>${escapeHtml(lastText)}</em></span>
        <span class="parallel-duel-side"><strong>${escapeHtml(getDuelScoreSummary(duel))}</strong><small>${escapeHtml(getDuelLeaderSummary(duel))}</small><i>${escapeHtml(status)}</i></span>
      </button>`;
    }).join('');
  ...
  function startNewParallelDuelDraft(){
    const current=getDuelState();
  ...
  function renderDuelSection(){
    ...
    const captain=document.getElementById('duelCaptainSelect');
    const opponent=document.getElementById('duelOpponentSelect');
    const options=(state.participants||[]).map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.avatar||'🎣')} ${escapeHtml(p.name)}</option>`).join('');
    [captain,opponent].forEach(sel=>{if(sel&&sel.innerHTML!==options)sel.innerHTML=options;});
  ...
  function updateDuelUi(){
    ...
    requestAnimationFrame(()=>{
      const activeSniper=getActiveSniperDuel();
      if(activeSniper?.active&&isSniperMode(activeSniper)){
        renderSniperSectionMap(activeSniper);
      }
    });
    if(typeof renderTournamentDuelSummary==='function')renderTournamentDuelSummary();
    renderDuelLeaderboard();
  }
  ...
  function clearSniperCatchMarkers(){if(sniperCatchLayer)sniperCatchLayer.clearLayers();}
  function renderSniperCatchMarkers(s){
    if(!duelMap||typeof L==='undefined')return;
    if(!sniperCatchLayer)sniperCatchLayer=L.layerGroup().addTo(duelMap);
    // Spot Sniper Elite intentionally does not render catch markers.
    // Catch locations are used only internally for hit detection.
    clearSniperCatchMarkers();
  }
  function initSniperSectionMap(){
    const el=document.getElementById('sniperSectionMap');

    if(!el||typeof L==='undefined')return false;

    if(sniperSectionMap&&sniperSectionMap.getContainer&&sniperSectionMap.getContainer()===el)return true;
    if(sniperSectionMap)destroySniperSectionMap();

    sniperSectionMap=L.map(el,{
      zoomControl:false,
      attributionControl:false,
      dragging:true,
      ...
      keyboard:false,
      tap:true
    }).setView([46.8182,8.2275],7);

    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{
      maxZoom:18,
      crossOrigin:true
    }).addTo(sniperSectionMap);

    sniperSectionLayer=L.layerGroup().addTo(sniperSectionMap);

    setTimeout(()=>sniperSectionMap?.invalidateSize(),80);

    return true;
  }
  function renderSniperSectionMap(s){
    const el=document.getElementById('sniperSectionMap');

    if(!el||!s?.active||!isSniperMode(s)||typeof L==='undefined')return;
    if(!initSniperSectionMap()||!sniperSectionLayer)return;

    sniperSectionLayer.clearLayers();
    ...
    spots.forEach(spot=>{
      ...
      const hit=!!spot.hit;
      const color=hit?'#35f28b':'#4ad7d1';
      const accent=hit?'#8ff0a7':'#ffb84d';
      const radius=Number(spot.radiusM||spot.radius||spot.baseRadiusM||30);

      L.circle([lat,lng],{
        radius,
        color,
        fillColor:color,
        fillOpacity:hit?0.28:0.12,
        weight:2,
        className:hit?'sniper-section-ring is-hit':'sniper-section-ring'
      }).addTo(sniperSectionLayer);

      L.circle([lat,lng],{
        radius:Math.max(4,radius*0.48),
        color:accent,
        fillColor:accent,
        fillOpacity:0.08,
        weight:1,
        className:hit?'sniper-section-inner is-hit':'sniper-section-inner'
      }).addTo(sniperSectionLayer);
      ...
      L.circleMarker([lat,lng],{
        ...
        fillOpacity:0.95,
        weight:2,
        className:hit?'sniper-section-core is-hit':'sniper-section-core'
      }).addTo(sniperSectionLayer);
    });

    if(bounds.length){
      sniperSectionMap.fitBounds(L.latLngBounds(bounds).pad(0.25),{
        maxZoom:17,
        animate:false
      });
    }
  ...
  function destroySniperSectionMap(){
    try{
      if(sniperSectionLayer){
        sniperSectionLayer.clearLayers();
      }

      if(sniperSectionMap){
        sniperSectionMap.remove();
      }
    }catch(e){
      console.warn('Sniper Section Map Cleanup fehlgeschlagen',e);
    }

    sniperSectionMap=null;
    sniperSectionLayer=null;
  }
  function updateDuelMap(){
    ...
    renderSniperCatchMarkers(s);
    const pts=(s.route||[]).filter(p=>Number.isFinite(Number(p.lat))&&Number.isFinite(Number(p.lng))).map(p=>[Number(p.lat),Number(p.lng)]);
  ...
  async function startDuel(){
    ...
    if(selectedMode==='sniper'){
      ...
      const spotsPerPlayer=Math.max(1,Math.min(3,Number(document.getElementById('sniperSpotsPerPlayerSelect')?.value||2)));
      s=ensureSniperState({
        ...defaultDuelState(),
        active:false,
        startedAt:null,
        ...
        mode:selectedMode,
        score:{[cap]:0,[opp]:0},
        sniperScore:{[cap]:0,[opp]:0},
        sniperSpots:[],
        sniperConfig:{spotsPerPlayer,currentTurn:0,players:[cap,opp],locked:false,setupDone:false,setupActive:true},
        lastTalk:`Spot Sniper Elite Setup: ${participantName(cap)} darf zuerst ${spotsPerPlayer} Spot${spotsPerPlayer===1?'':'s'} setzen.`
      });
      saveDuelState(s);
      stopTimers();
      updateDuelUi();
  ...
  async function endActiveSniperDuel(){
    let s=getActiveSniperDuel();
    if(!s?.active)return;
    s.active=false;
    s.endedAt=new Date().toISOString();
    s.route=[];
    s=rebuildSniperScoreFromHitSpots(s);
    s.routeSnapshotSvg=buildSniperPreviewSvg(s);
    s.fishImage=null;
    s.imageUrl=null;
  ...
  async function cancelActiveSniperDuel(){
    ...
    if(!s?.active)return;
    if(!confirm('Spot Sniper Elite wirklich abbrechen? Fänge bleiben erhalten, das Mini-Game wird nicht als normales Ergebnis gewertet.'))return;
...
function duelGpsErrorMessage(error){
  if(!error)return 'Standort konnte nicht ermittelt werden. Bitte GPS und Berechtigung prüfen.';
  if(error.code===1)return 'GPS-Berechtigung verweigert. Bitte Standortzugriff erlauben.';
  if(error.code===3)return 'GPS-Timeout. Bitte freien Himmel prüfen und erneut versuchen.';
  return 'Standort konnte nicht ermittelt werden. Bitte GPS und Berechtigung prüfen.';
}
function setDuelGpsMessage(message){
  const meta=document.getElementById('duelMapMeta');
...
async function addGpsPoint(){
  ...
  let point=null;
  try{
    point=await getFreshDuelPosition(9000);
  }catch(e){}
...
async function forceCurrentDuelLocation(){
  ...
  function getDefaultDuelCatcherId(s){
    return s?.captainId||document.getElementById('duelCaptainSelect')?.value||s?.opponentId||'';
  }
  ...
  async function addDuelCatch(species){
    let s=getDuelState();if(!s.active){alert('Starte zuerst ein Duell.');return;}
    const fish=fishTiles.find(f=>f.species===species)||fishTiles[fishTiles.length-1];
    ...
    const catchEvent={id:crypto.randomUUID(),participantId:target,species:fish.species,points:fish.points,at:new Date().toISOString()};

    s.score=s.score||{};
    s.score[target]=Number(s.score[target]||0)+fish.points;
    s.catches=[...(s.catches||[]),catchEvent];
    s.lastTalk=`${participantName(target)} legt ${fish.species} vor. +${fish.points} Punkte – der Kescher applaudiert.`;
