...
function loadTournamentIntoForm(t){showScreen('setup');const form=document.getElementById('tournamentForm');form.dataset.editingId=t.id;form.name.value=t.name||'';form.start.value=t.start||'';form.end.value=t.end||'';document.getElementById('rulesetSelect').value=t.rulesetId==='custom'?'all_fish':(t.rulesetId||'all_fish');document.getElementById('enableCustomRules').checked=t.rulesetId==='custom';const rules=t.customRules||{};['pointsPerFish','bonusFirstFish','bonusLargestFish','bonusLargestPerSpecies','bonusNewArea','bonusOver80cm','bonusOver100cm'].forEach(key=>{const el=document.getElementById('rule_'+key);if(el)el.value=rules[key]??el.defaultValue??0});document.querySelectorAll('#tournamentParticipants input[type="checkbox"]').forEach(cb=>{cb.checked=(t.participantIds||[]).includes(cb.value)});const submit=form.querySelector('button[type="submit"]');if(submit)submit.textContent='Turnier speichern';updateRulesPreview();window.scrollTo({top:0,behavior:'smooth'})}
